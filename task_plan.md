...
- [ ] 更新中文阶段报告
- **状态：** official_v4 8 样本探针通过；全量 2,360 样本运行中，已完成 10–290 分钟进度记录，22:40:20Z runner 报告 280/2360，服务仍持续完成请求
...
- [ ] 更新中文阶段报告
- **状态：** 隔离分支已完成配置/spec、runtime artifact fail-closed 加载、GPU batch metadata，以及三池 write/demotion/DSA mixed read 的代码接线；最新 commit `f426ab5a5` 已推送，79 项测试通过、24 项 A800 测试跳过，并显式拒绝 V2 runner、非 eager、CUDA graph、speculative、DCP、PCP、DBO、async scheduling、KV transfer 和 KV offloading；真实 GLM‑5.2 EngineConfig 已验证显式关闭 async 后成功且未初始化 CUDA，CPU interpreter 已实际覆盖非连续 rotation stride，并分别对输出与自然对数域 LSE 做 PyTorch oracle 对比；A800 门禁已加入 chunked/one-shot 最终分区一致性，INT2 reference 已覆盖 constant/narrow/random/outlier；A800 实际 launch 和服务端到端尚未验证
