...
- [ ] 当前进行中：split-K stride最小连续临时输出+copy-back补丁已由source `ea8ae6b7758ae2b4db7cae44d638ae5de80148ac`发布。新Phase6 OCI、递归verifier、overlay source import和daemon导入均已闭合；新Stage9 control `oscar-glm-stage9-runtime:ea8ae6b77` / `sha256:ad0f218b...ebf4`的继承审计10/10、build manifest 12/12及CPU runtime preflight 22/22、runtime manifest 13/13均通过，正式记录已实时追加2.258并完成门禁，当前先发布。发布后完成新的GPU双空闲门禁和driver-visible import，迁移Phase5/7/9活动身份，再运行同一固定256题精度门禁。OSCAR达到BF16固定256题105/256前不得复测32K/batch1性能；达到后才在同负载对比TTFT/TPOT并继续优化。
