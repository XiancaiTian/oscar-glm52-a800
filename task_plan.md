...
- [x] 20分钟固定节点为16/256完成、11正确、68.750000%当前精度/4.296875%全量精度、0 invalid/0 truncated；截止逐题复算一致，23/23 validation与9/9 manifest通过。
- [x] 20分钟节点已实时写入统一报告2.15：报告2.1–2.18连续，8项证据大小/hash、术语、交叉引用、whitespace与diff门禁通过。
- [x] 30分钟固定节点仍为16/256完成、11正确、68.750000%当前精度/4.296875%全量精度、0 invalid/0 truncated；8卡均98%，运行态健康，23/23 validation与9/9 manifest通过。
- [x] 30分钟节点已实时写入统一报告2.15：报告2.1–2.18连续，8项证据大小/hash、术语、交叉引用、whitespace与diff门禁通过。
- [x] 隐藏层代理离线比较器CPU阶段闭合：两次有效红灯、目标5/5、Phase9递归95/95、ruff、25/25 validation与15/15 manifest通过；输出强制`promotion_gate=false`。
- [x] 代理工具已作为统一报告2.18独立小节记录，原当前结论顺延为2.19并修正交叉引用；12项核心证据大小/hash通过。
- [x] 40分钟固定节点仍为16/256完成、11正确、68.750000%当前精度/4.296875%全量精度、0 invalid/0 truncated；8卡97%–100%，23/23 validation与9/9 manifest通过。
...
- [x] 70分钟节点、position过滤实现/测试、统一报告与planning已由主仓`173d8e5`通过GitHub HTTPS发布，local/upstream一致。
...
- [x] 110分钟节点已由`bac78943a514faaaff37e910471c1aa97db96469`提交并通过GitHub HTTPS发布，local/upstream一致。
...
- [x] 130分钟节点已由`3d6d398`提交并通过GitHub HTTPS发布，local/upstream一致。
- [x] 140分钟固定节点仍为102/256完成、45正确、44.117647%当前精度/17.578125%全量精度、0 invalid/48 truncated；8卡98%–100%且运行态健康，23/23 validation与9/9 manifest通过。
- [x] 140分钟节点已实时写入统一报告：2.1–2.19连续，证据hash、术语、交叉引用、whitespace与diff门禁通过。
- [x] 140分钟节点已由`1081110`提交并通过GitHub HTTPS发布，local/upstream一致。
- [x] 150分钟固定节点为109/256完成、45正确、41.284404%当前精度/17.578125%全量精度、0 invalid/55 truncated；8卡97%–98%且运行态健康，23/23 validation与9/9 manifest通过。
- [x] 150分钟节点已实时写入统一报告：2.1–2.19连续，证据hash、术语、交叉引用、whitespace与diff门禁通过。
...
- [x] 200分钟固定节点为155/256完成、62正确、40.000000%当前精度/24.218750%全量精度、0 invalid/80 truncated；8卡98%–100%且运行态健康，23/23 validation与9/9 manifest通过。
- [x] 200分钟节点已实时追加到统一报告：2.1–2.19连续，8/8报告证据哈希、23/23 validation、9/9 manifest、术语、交叉引用和diff门禁通过。
- [x] 200分钟节点、统一报告与planning已由`526c4c4`提交并通过GitHub HTTPS发布，local/upstream一致。
- [x] 210分钟固定节点仍为155/256完成、62正确、40.000000%当前精度/24.218750%全量精度、0 invalid/80 truncated；8卡98%–100%且运行态健康，23/23 validation与9/9 manifest通过，判定为长输出批次而非卡死。
- [x] 210分钟节点已实时追加到统一报告：2.1–2.19连续，8/8报告证据哈希、23/23 validation、9/9 manifest、术语、交叉引用和diff门禁通过。
- [x] 210分钟节点、统一报告与planning已由`02dbd75`提交并通过GitHub HTTPS发布，local/upstream一致。
- [ ] 当前进行中：实验继续由monitor每600秒自动打印，下一固定节点为13:52:29 CST。当前轮自然结束并释放GPU后，再执行BF16/OSCAR同token hidden capture与代理标定；正式capture前仍需处理无关未跟踪文件对formal clean门禁的影响。
