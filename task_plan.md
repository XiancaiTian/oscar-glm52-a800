...
- [ ] 当前进行中：833固定256题按10分钟监控；370分钟节点223/256、0正确，正式容器健康运行。split-K stride CPU TDD候选及含短行sentinel覆盖的GPU最小复现脚本已持久化，4项manifest通过；尚待正式run终局后的GPU native数值门禁与修复后同256题，补丁未提交/发布/迁移。
