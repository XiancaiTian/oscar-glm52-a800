...
对冻结预测做只读诊断后发现，717/1,319 个 GSM8K 输出恰好达到 1,024-token
completion 上限，其中 549 条被判错；未达到上限的 602 条中有 498 条通过冻结
评分，即 `0.8272425249169435`。此外，101 条冻结判错记录在只提取首个数值并去除
单位、货币符号等格式后与 gold 数值一致，例如 gold `540` 对模型输出
`540 meters`。按该诊断规则可对齐 767/1,319（`0.5815011372251706`），但它不是
冻结 evaluator 的正式结果，不能替代 666/1,319。上述证据说明差异同时受输出触顶/
重复和严格答案规范化影响；在没有复现用户 86.35% 所用配置前，不把剩余差异归因于
单一因素。
