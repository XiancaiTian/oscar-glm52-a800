...
`0.002/0.002`。轮次没有进入 warm-up 或计时，也没有生成 `result.json`，
因此没有 hybrid 性能结果。独立 cache 为 61 个文件，日志 SHA256 为
