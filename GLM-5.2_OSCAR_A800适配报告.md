...
- grouped prefill 单卡单层实验已把 cropped top-k/split1 从
  ...
  `cuda_initialized=false`。正式 32K/batch1 单格随后以退出码 0 完成，
  三轮 `mean` 中位数为 TTFT `47,143.207 ms`、TPOT `199.458 ms`、
  ...
  固定单卡 2,048×2,048 筛选进一步实测 grouped split1 为 `24.090 ms`，
  相对同轮 split16 加速 `8.128×`，相对旧 4-warps 的 `26.906 ms` 再降低
  `10.47%`；冻结 output/LSE allclose 均通过，实际 cubin 从
  255 registers、656-byte stack 降至 247 registers、0-byte stack，
  shared memory 保持 135,168 bytes。完整 cold-cache CUDA 回归随后为
  125/125 passed、0 skipped/failed；该候选仍需新 OCI 与新的
  32K/batch1 端到端实测，之后才能冻结并完成同提交完整矩阵；
- 128K 候选扩展验证尚未完成。
...
2. 提交 `a0171ed6a` 将 `num_stages` 降为 1 后成功编译，但轮次
   ...
   `torch.allclose(atol=0.002, rtol=0.002)` 协议的允许范围，因此没有
   放宽门限，也没有进入计时；
...
方案；仍以 output/LSE 分别执行
`torch.allclose(atol=0.002, rtol=0.002)` 为冻结门限，同时披露
max_abs/max_rel，不以性能为由放宽精度。候选通过后再构建正式 OCI，并以新
run ID 重跑 32K/batch1。
...
CPU-only 的 SM80 离线资源 sweep
`20260731T0400Z_tf32_offline_resource_sweep_v1` 随后复用实际 GPU 轮次生成的
TTIR，以固定 `num_warps=4/num_stages=1` 重编译。全 IEEE/全 TF32 分别需要
`139,264/169,984 bytes`；简单把一部分 dot 改回 IEEE 反而需要
`169,984–204,800 bytes`。只让原始输入本来就是 BF16 的
query/prefix/recent/RoPE 路径使用 BF16 tensor core，同时让 rotation/INT2
反量化后的 history score/value 保持 TF32 时，资源降至 `135,168 bytes`，
比苹果800上限低 `31,744 bytes`。summary 状态为 `passed`，SHA256 为
`1065b8416b2424a9c0fa48dce8c6a3b134f0cb1f7d6bc54a535481394b6136db`；
该轮没有分配 GPU，只证明离线编译资源预算。

上述 hybrid 已由源码提交
`b9626ce9fdd627da23fd29629ceb09df83e1458b` 落地并推送，Git tree 为
`ecf196e90160a0a9b6d1e72bc4d7de6b946d9f0b`。它只修改 grouped prefill
kernel 一个文件，共 7 行新增、8 行删除：原生 BF16 pool 与 RoPE 使用 BF16
tensor core，history score/value 使用 TF32，softmax/global LSE 与最终
accumulator 保持 FP32；decode、三段式 cache、write/demotion 和调度不变。

固定控制环境中的 Triton interpreter/head-block 定向测试为 6/6 passed；
源码提交时 ruff check/format、typos、mypy、SPDX、forbidden imports、
CUDA API、attention backend 文档和 sign-off 等全部适用 hooks 通过。当前
kernel 源码 SHA256 为
`e7f9d1142610b96726c0488679138185398c2f7c1fca71fcbfaa655635982629`。
主仓库 `96cd9c2133e96561ac4fb7db1b2b4bef8fa4e7ed` 发布该 submodule 与记录后，
单卡轮次 `20260731T0408Z_prefill_2k_hybrid_v1` 前两次 8/8 空闲检查为
`04:06:50Z/04:07:56Z`，间隔 66 秒。实际编译/launch 的 grouped stage1
shared memory 为 `135,168 bytes`，与离线结果一致。

但 hybrid 被固定正确性门禁拒绝：相对同轮 IEEE split16，output/LSE 最大绝对
误差为 `0.004933357238769531/0.0020360946655273438`，超过
冻结的 `torch.allclose(atol=0.002, rtol=0.002)` 允许范围，其中 output
未通过。轮次没有进入 warm-up 或计时，也没有生成 `result.json`，因此没有
hybrid 性能结果。独立 cache 为 61 个文件，日志 SHA256 为
...
`619ea47d74296e77e1357858a53d3aaf11e349d6`。源码只修改 grouped prefill
kernel 的 BF16 value 累加：probability 保持 FP32，BF16 value 在 dot 输入
处扩展为 FP32 并使用 TF32；BF16/RoPE score、history、softmax/LSE、decode
和三段式 cache 均不变。实际 diff 为 1 个文件、3 行新增、2 行删除。

固定 CPU 容器中的 prefill head-block 与 Triton interpreter 定向测试为
6/6 passed、10.27 秒；ruff check/format 和提交时全部适用 hooks 均通过。
kernel 源码 SHA256 为
`978b260511a8a1aa6dd822f0eff8196a519982e454f694dba18335c9b09e78a0`。
源码仓库本地与远端一致，本阶段没有分配 GPU；新源码尚未产生 GPU output/LSE
或性能结果。

主仓库 `329962cc3c35e9db0cd1cf11a32c603b66d0c6ce` 发布该源码与记录后，
轮次 `20260731T0422Z_prefill_2k_value_precision_v1` 前两次 8/8 空闲检查为
`04:19:54Z/04:21:07Z`，间隔 73 秒；实验固定只使用 GPU 0。实际 grouped
stage1 shared memory 为 `135,168 bytes`，独立 Triton cache 为 61 个文件。

该轮按自 benchmark 首次提交 `60acb2e8` 起冻结的
`torch.allclose(atol=0.002, rtol=0.002)` 协议状态为 `passed`，完成每个
配置 5 次 warm-up 与 7 次正式测量：
...
`201.39968872070312/0.0002782414376270026`。这些诊断值与 allclose 状态同时
记录，不能把协议通过误写为 `max_abs<=0.002`。result/log SHA256 分别为
...
该复现入口先由主仓库提交
`eeaf56c81b7c65116a84bf59d3467e4185c98aa5` 发布，再执行 CPU-only
构建。有效目录为
`artifacts/phase9-control/20260731T0508Z_runtime_b247211c9_v1`，
控制镜像 tag/image ID 为：

- `oscar-glm-stage9-runtime:b247211c9`；
- `sha256:edbbc87d609963abd05b3f55c81d0ecad7016f7f31a860f6a62fa74f2bb7b1b8`。

控制镜像共 34 层，前 33 层与 base
`sha256:8053b791ca3de5a7f2f47ac79ab35f981931b6a2b99848e0c7123c30d79a9e46`
逐层相同，全部 inherited labels 也完全匹配。CPU runtime 检查确认：

- Git `2.34.1`、iproute2 `5.15.0`；
- Python/glibc `3.12.13/2.35`；
- `/opt/phase9-control-packages.txt` 与实际包版本一致；
- `cuda_initialized=false`。

首次身份审计脚本把构建日志中的 12 位短 ID 扩写成猜测的完整 ID，因错误断言
退出；这不代表镜像身份失败，也没有分配 GPU。v2 改为从 daemon 读取完整 ID
后，34/33 层、基础层逐层继承、labels 与 entrypoint 全部通过。build log、
...
process。新候选的正式运行前门禁已经完成；随后执行的正式结果见 7.17。

### 7.17 Value 精度恢复候选的 32K/batch1 正式结果

正式单格轮次
`20260731T0536Z_stage9_candidate_b247211c9_32k_b1_v1` 使用主仓库提交
`1d32d26cfba5239886cba6fa8781acd7f1461f48`、源码提交
`b247211c91cd787149123f0373945e8a0c6c9937` 和 Phase 9 配置 SHA256
`e2c764d75e370a8c7784f1807faaf9ab674c3fc0e9b5ed8c17188c1d73349114`。
负载固定为 32,768 输入 token、128 输出 token、batch/并发 1、1 次
warm-up、3 轮正式测量和 8+8+1 profiler。

外层双空闲检查为 `05:34:03Z/05:35:12Z`，间隔 69 秒；容器内双空闲检查为
`05:37:14Z/05:38:18Z`，间隔 64 秒。四次检查均为 8 张 GPU 0 MiB、0%
且没有 compute process。141/141 权重分片全部加载，模型加载耗时
`272.713535 秒`、每卡模型内存 `56.0 GiB`、可用 KV cache
`13.74 GiB`。启动、服务和 profiler 均实际打印 10 分钟进度。
...
当前候选的 TPOT 已在相对 BF16 的 20% 门限内，但 TTFT 仍为 BF16 的约
`3.76×`，不能进入最终完整矩阵结论。
...
为 3，kernel total 为 `78,446 ms`。8-rank table 中，
`_mixed_sparse_prefill_stage1` 均执行 1,248 次，CUDA total 中位数为
`34,398 ms`，即 `27.563 ms/层/chunk`；相对上一候选的
`93,913.327 ms` 下降 `63.37%`，但仍为 BF16 同项
`3,384.374 ms` 的约 `10.16×`。服务端最多运行 1 个请求、等待为 0、
preemption 为 0，KV usage 峰值为 `5.7902%`，不存在容量排队。
...
完成间隔 66 秒的两次 8/8 GPU 空闲检查；实验固定只使用 GPU 0，并沿用
2,048 query、2,048 top-k、每 rank 8 heads、latent 512、
prefix/history/recent `64/1728/256`、5 次 warm-up、7 次正式测量和冻结
...
8-warps grouped split1 相对同轮 split16 加速 `8.128×`，并比旧 4-warps
grouped split1 的 `26.906 ms` 再降低 `10.47%`。output/LSE 均通过冻结
allclose；诊断 max_abs 为
`0.004912614822387695/0.0020360946655273438`，max_rel 为
`201.39968872070312/0.0002782414376270026`，与 b247 候选一致。

实际 SM80 cubin 为 247 registers、0-byte stack，shared memory 为
135,168 bytes；相对 4-warps 的 255 registers、656-byte stack，寄存器和
spill 均下降。cubin/PTX/metadata JSON SHA256 分别为
...
容器退出后 `06:45:45Z` 复查 8 张 GPU 均为空闲。

单卡阶段记录由主仓库提交 `9b5d81d` 发布后，完整 CUDA v1
`20260731T0655Z_8warps_full_cuda_v1` 因错误假设 uv 位于
`/opt/uv/bin/uv`，在 pytest 启动前以 127 退出；0 测试、0 Triton cache，
不计为 CUDA 结果。失败 pytest/exit-code 文件 SHA256 分别为
...
CPU-only 探针确认实际 uv 为 `/usr/local/bin/uv`。失败记录由主仓库提交
`4971f21` 发布后，`06:54:58Z/06:56:21Z` 两次 8/8 GPU 空闲检查间隔
83 秒。有效轮次 `20260731T0657Z_8warps_full_cuda_v2` 固定只使用 GPU 0，
源码和 phase0 native rootfs 只读挂载，并使用独立空 Triton cache。结果为
125 passed、0 skipped、0 failed、18 warnings、78.19 秒；cold cache
380 个文件、内容合计 25,886,842 bytes。pytest/exit-code SHA256 分别为
`bd3b6d624a79c5867279f28dcb924c56d8a9a812df5ad8ae4cc5538c1a0f63a1` /
`9a271f2a916b0b6ee6cecb2426f0b3206ef074578be55d9bc94f6f3fe3ab86aa`。
容器自动删除，`06:58:25Z` 复查 8 张 GPU 均为空闲。

至此该候选已关闭单层资源、冻结精度、性能筛选和完整 cold-cache CUDA
正确性门禁；尚未生成新 OCI，也没有新的 32K/batch1 端到端结果。
