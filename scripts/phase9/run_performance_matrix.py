...
class MatrixRunner:
    ...
    def validate_preflight(self) -> dict[str, Any]:
        ...
        expected = {
            ...
            "max_num_batched_tokens": self.config["server"]["max_num_batched_tokens"],
            "kv_cache_dtype": expected_dtype,
            "gpu_memory_utilization": self.config["server"]["gpu_memory_utilization"],
            "attention_backend": self.config["server"]["attention_backend"],
            "seed": self.config["server"]["seed"],
