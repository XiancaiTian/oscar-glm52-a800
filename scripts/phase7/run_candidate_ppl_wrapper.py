#!/usr/bin/env python3
"""Run the frozen WikiText-2 runner with the Stage 7 OSCAR candidate."""
...
def main() -> int:
    ...
    def build_candidate_llm(
        *,
        ...
        if tensor_parallel_size != 8:
            raise ValueError("Stage 7 perplexity requires tensor_parallel_size=8")
        if not enforce_eager:
            raise ValueError("Stage 7 perplexity requires eager execution")
        if kv_cache_dtype != "oscar_mla_int2":
            raise ValueError(
                f"Stage 7 perplexity requires oscar_mla_int2, got {kv_cache_dtype}"
            )
        return LLM(
            model=model_path,
    ...
    module.build_llm = build_candidate_llm
    sys.argv = [
        str(args.runner),
        "--suite-dir",
        str(args.suite_dir),
