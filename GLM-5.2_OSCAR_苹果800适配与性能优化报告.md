...
启动后7,800秒固定截止为2026-08-04 12:22:29 CST：完成102/256、正确45题，当前
完成集精度44.117647%，全量精度17.578125%，0 invalid、48 truncated。较120分钟
节点新增14个完成样本、其中10题正确；该中间值仍不能外推终局，固定256题
门禁仍以终局正确数是否达到105为准。截止逐题复算与monitor一致，容器、tmux、
c16 runner、EngineCore和8个TP worker均存活，实验继续运行。23/23 validation与
9/9 manifest一次通过。证据仍位于
`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_130min.log` | 151 bytes | `5eef03e569bfc6e69bbb063b12e5b6440d23fa1271c689a9f879f6c0ea0fdae8` |
| `checkpoint_130min_cutoff_rows.json` | 1,366,005 bytes | `dcd14890f44d84c93f6292073eaf85c7fa078068beb88bbff1688fffc5283269` |
| `checkpoint_130min_gpu.csv` | 107 bytes | `9fe6895e3d8d1ec140239db730c3e8121e11672d5da4d44b2a09e74893902eaf` |
| `checkpoint_130min_compute.csv` | 600 bytes | `d9f39a98d0d4550dce974df7b12288871aeb67a454f5ffdd860a00ffe0d83f11` |
| `checkpoint_130min_runtime_state.txt` | 3,943 bytes | `399257d65ea8e51f5b4c91547e7767880839d514e1e9a977d931fe131a8757fc` |
| `checkpoint_130min_validation.json` | 3,985 bytes | `1b96abbbf9cbe81362a1faf2e69d2bc4e804b3bf5c67ac00db9d2f3fbf943654` |
| `checkpoint_130min_manifest.sha256` | 869 bytes | `6958f812190c84cf140b5024625a1a1e108c01dbd5745f05f70eff62ca7f39d2` |
| `checkpoint_130min_manifest_check.txt` | 325 bytes | `0d003fad95e5d00c99f92a099b36a0b1aa6bbaafe1fced92e4214181233fb17c` |

启动后8,400秒固定截止为2026-08-04 12:32:29 CST：仍为102/256完成、正确45题，
当前完成集精度44.117647%，全量精度17.578125%，0 invalid、48 truncated；相对
130分钟节点没有新增完成题。截止逐题复算与monitor一致；现场8卡利用率为
98%–100%，容器、tmux、c16 runner、EngineCore和8个TP worker均存活，因此判定为
长输出批次仍在生成，不是实验卡死，不重启。23/23 validation与9/9 manifest一次
通过。证据仍位于
`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_140min.log` | 151 bytes | `f7d079fd48774aa7180d44bfc979b3a23770be78c9c44f46f06ddc3e6362faa2` |
| `checkpoint_140min_cutoff_rows.json` | 1,366,005 bytes | `dcd14890f44d84c93f6292073eaf85c7fa078068beb88bbff1688fffc5283269` |
| `checkpoint_140min_gpu.csv` | 106 bytes | `318cc0a19666c34b4ca50c8da8e55e3e2f0566cbf2122ba59f102fd80fac4111` |
| `checkpoint_140min_compute.csv` | 600 bytes | `d9f39a98d0d4550dce974df7b12288871aeb67a454f5ffdd860a00ffe0d83f11` |
| `checkpoint_140min_runtime_state.txt` | 3,943 bytes | `c42175bdd66f9399dc6f0f4167def067870d876366010128828ff761a8a168d7` |
| `checkpoint_140min_validation.json` | 3,985 bytes | `57d2bf8bb95a3eb69797d97cfe30c161ccce14a1371e7aafd3f2febff1338cd4` |
| `checkpoint_140min_manifest.sha256` | 869 bytes | `bcf63583939a052227c8f021760431d6e9b14993aa4097b3cf036c94918de74d` |
| `checkpoint_140min_manifest_check.txt` | 325 bytes | `246d37f3c113bda619adbf4186bfa9efd671ae4ca728666de1aa181439db60cc` |

启动后9,000秒固定截止为2026-08-04 12:42:29 CST：完成109/256、正确45题，当前
完成集精度41.284404%，全量精度17.578125%，0 invalid、55 truncated。较140分钟
节点新增7个完成样本，本节点正确数未增加，truncated数增加7。该中间值仍不能
外推终局，固定256题门禁仍以终局正确数是否达到105为准。截止逐题复算与
monitor一致；现场8卡利用率为97%–98%，容器、tmux、c16 runner、EngineCore和8个
TP worker均存活，实验继续运行。23/23 validation与9/9 manifest一次通过。证据
仍位于`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_150min.log` | 151 bytes | `7ad8f3922edf0687c325190a2d7f964641e7cfa9e6c82d706b41dec052157f67` |
| `checkpoint_150min_cutoff_rows.json` | 1,549,966 bytes | `66dd5fe679f5ea598ec56b13f98c09a872d6409ce2ec2e920a13801a083e134f` |
| `checkpoint_150min_gpu.csv` | 104 bytes | `44b7213fc3e7347dd2a993500327a9955a532614124964e4b643dc357d037a49` |
| `checkpoint_150min_compute.csv` | 600 bytes | `d9f39a98d0d4550dce974df7b12288871aeb67a454f5ffdd860a00ffe0d83f11` |
| `checkpoint_150min_runtime_state.txt` | 3,943 bytes | `fc8cb2887c3f037536b3c156e2d57ec9b5f6eda5f9e8dfa7988d4d97598f8782` |
| `checkpoint_150min_validation.json` | 3,985 bytes | `532eda2c7879043c6ff5eaae856538fd7bbdce1accf03fe9e584cd13727ca294` |
| `checkpoint_150min_manifest.sha256` | 869 bytes | `2fe45adc5b76e37e26f73bf62ccd8023439e2a763934c99603be5c0b6d27a8e7` |
| `checkpoint_150min_manifest_check.txt` | 325 bytes | `d27f4556012149477a1e3c37e1d930c9b67e4932539711609224e42bae0a7e6f` |

启动后9,600秒固定截止为2026-08-04 12:52:29 CST：完成123/256、正确49题，当前
完成集精度39.837398%，全量精度19.140625%，0 invalid、63 truncated。较150分钟
节点新增14个完成样本、其中4题正确，truncated数增加8。该中间值仍不能外推终局，
固定256题门禁仍以终局正确数是否达到105为准。截止逐题复算与monitor一致；现场
8卡利用率均为98%，容器、tmux、c16 runner、EngineCore和8个TP worker均存活，实验继续
运行。23/23 validation与9/9 manifest一次通过。证据仍位于
`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_160min.log` | 151 bytes | `ff6e29d79acae3b6603898b61b307d06d3e97947e18cff279dbfd1a2ce6b8714` |
| `checkpoint_160min_cutoff_rows.json` | 1,785,294 bytes | `c02179d943361a3c5a21853bda5fbc9feb81d85952c3a9da7fec02541da22098` |
| `checkpoint_160min_gpu.csv` | 104 bytes | `a85565d1aa2e0c8108653860e319998a72e88c28818eff2a90b3dee602e55ef9` |
| `checkpoint_160min_compute.csv` | 600 bytes | `4e04414efaf87f1041905f92c43106f45acb1ac3acf06038fb8a8e5b27bd2ab2` |
| `checkpoint_160min_runtime_state.txt` | 3,943 bytes | `1961b271f0c52cbb48c73e50fd734037f5df4620fcd30a4529cbce9b0786b513` |
| `checkpoint_160min_validation.json` | 3,985 bytes | `ea324bc756aa289a8996bf2cc45b42cbeb8596877d8478f5c0bdda2cf8dfcd18` |
| `checkpoint_160min_manifest.sha256` | 869 bytes | `5a6854cde6af3aead1a21d50a4380fde17b3a09a110c4a5f98a34bc6752ed7ad` |
| `checkpoint_160min_manifest_check.txt` | 325 bytes | `92b253344632a65d67513dc90073a37862b153d1039aff44eec205b52ff8b87b` |

启动后10,200秒固定截止为2026-08-04 13:02:29 CST：完成124/256、正确49题，当前
完成集精度39.516129%，全量精度19.140625%，0 invalid、64 truncated。较160分钟
节点仅新增1个完成样本，正确数未增加，truncated数增加1。该中间值仍不能外推终局，
固定256题门禁仍以终局正确数是否达到105为准。截止逐题复算与monitor一致；现场
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_170min.log` | 152 bytes | `a7d0c470c979efa4aa593d3082a4a083407c8c295d01e2594e0c46c9121f692b` |
| `checkpoint_170min_cutoff_rows.json` | 1,813,511 bytes | `2ad4aa66a9e19a0c2cee559ca5dca1c38f8a042f93826349827f682daaa852e6` |
| `checkpoint_170min_gpu.csv` | 104 bytes | `252876c2f2a394aead2a46940ff14f3e9cd2609a1ecffbac3b3e1e6201d38de7` |
| `checkpoint_170min_compute.csv` | 600 bytes | `4e04414efaf87f1041905f92c43106f45acb1ac3acf06038fb8a8e5b27bd2ab2` |
| `checkpoint_170min_runtime_state.txt` | 3,943 bytes | `6257ba400c0866242d479d0cd6d0cf823d7b6e1805df54c8dfecb559008b16df` |
| `checkpoint_170min_validation.json` | 3,987 bytes | `afee05f087d810c9465703d95f510dcb3bd45270a7fd9b56d6c63b5b2ad29282` |
...
启动后11,400秒固定截止为2026-08-04 13:22:29 CST：完成149/256、正确57题，当前
完成集精度38.255034%，全量精度22.265625%，0 invalid、79 truncated。较180分钟
节点新增23个完成样本、其中8题正确，truncated数增加13。该中间值仍不能外推终局，
固定256题门禁仍以终局正确数是否达到105为准。截止逐题复算与monitor一致；现场
8卡利用率为85%–98%，容器、tmux、c16 runner、EngineCore和8个TP worker均存活，实验继续
运行。23/23 validation与9/9 manifest一次通过。证据仍位于
`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_190min.log` | 152 bytes | `4a4b09de38f8a306808b61b28414c57d80f650b6213a235545d3e29f30bd9a74` |
| `checkpoint_190min_cutoff_rows.json` | 2,214,799 bytes | `8f75c9382da944ef31614e59df93decbfa714040ee3f03bb755cf86261e9b960` |
| `checkpoint_190min_gpu.csv` | 104 bytes | `28814cc669f3e8d725287d76e67dbf99c50231016fda9e3fb2f90b5caa32fb9d` |
| `checkpoint_190min_compute.csv` | 600 bytes | `0d1b9ec2937b55dd7eca692b7b0405188b066952aaef5eb00c316c502c00627e` |
| `checkpoint_190min_runtime_state.txt` | 3,943 bytes | `0787e0c4a0cb2b3c73ec19fff572a558c4c0b20e875dc87072296d68e772aa7f` |
| `checkpoint_190min_validation.json` | 3,987 bytes | `3ee75c04f4ef32b14091e4f09c52cb3609cc454f8a601cd4d32873bc85efe8fd` |
| `checkpoint_190min_manifest.sha256` | 869 bytes | `4a5e1ed311dd1f8608c01914cf32a74144f2b89685bedeafc633c434120443df` |
| `checkpoint_190min_manifest_check.txt` | 325 bytes | `16f3f0b300139e6bf45919a9888a14c01d6510e063e67c8ac233f153c333a7b7` |

启动后12,000秒固定截止为2026-08-04 13:32:29 CST：完成155/256、正确62题，当前
完成集精度40.000000%，全量精度24.218750%，0 invalid、80 truncated。较190分钟
节点新增6个完成样本、其中5题正确，truncated数增加1。该中间值仍不能外推终局，
固定256题门禁仍以终局正确数是否达到105为准。截止逐题复算与monitor一致；现场
8卡利用率为98%–100%，容器、tmux、c16 runner、EngineCore和8个TP worker均存活，实验继续
运行。23/23 validation与9/9 manifest一次通过。证据仍位于
`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_200min.log` | 152 bytes | `450724a16ba97b6ad7b37ecb7f9ffaffb92d0713ed15527e4807c56b331d39fa` |
| `checkpoint_200min_cutoff_rows.json` | 2,243,524 bytes | `bd2a60aa324ec8b32c38abfbb9d089fa4ccb3cb76df20bc70fc7c0ca5c763b2f` |
| `checkpoint_200min_gpu.csv` | 105 bytes | `6d724b2baaa4c743aa2e9f5ada3c645d636913e06bc3227f74b11c68393a816e` |
| `checkpoint_200min_compute.csv` | 600 bytes | `0d1b9ec2937b55dd7eca692b7b0405188b066952aaef5eb00c316c502c00627e` |
| `checkpoint_200min_runtime_state.txt` | 3,943 bytes | `e23ba3e6188e9fdaa4f84a7f74d1067fc58a9e5cab2ad44ecbb0fb5576854bf6` |
| `checkpoint_200min_validation.json` | 3,987 bytes | `f54261c7484ba6e7abe78110a2c5417896646d87c12ce38f8fa93c024090b5b8` |
| `checkpoint_200min_manifest.sha256` | 869 bytes | `52d5fb12c715edcbec22138bc4c8663c1766e2ae0045fc394d548307f5a2620c` |
| `checkpoint_200min_manifest_check.txt` | 325 bytes | `f704a9c35210265d273c79ba3bf28328e9f93a14323a90a01c1a41c32336106a` |

启动后12,600秒固定截止为2026-08-04 13:42:29 CST：仍为155/256完成、正确62题，
当前完成集精度40.000000%，全量精度24.218750%，0 invalid、80 truncated；相对
200分钟节点没有新增完成题。截止逐题复算与monitor一致；现场8卡利用率为
98%–100%，容器、tmux、c16 runner、EngineCore和8个TP worker均存活，因此判定为
长输出批次仍在生成，不是实验卡死，不重启。23/23 validation与9/9 manifest一次
通过。证据仍位于
`artifacts/phase9-control/20260804T0212Z_ea8_topk2048_fast256_launch_v1`：
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_210min.log` | 152 bytes | `5128b3390ea5097341c76fd76460a9239dc14b0fcfffbbff4395bc45247c10de` |
| `checkpoint_210min_cutoff_rows.json` | 2,243,524 bytes | `bd2a60aa324ec8b32c38abfbb9d089fa4ccb3cb76df20bc70fc7c0ca5c763b2f` |
| `checkpoint_210min_gpu.csv` | 106 bytes | `8c6434554dcc26456c03596afddabb3508a633275a51d4cdcbaa0f95b092ef4d` |
| `checkpoint_210min_compute.csv` | 600 bytes | `0d1b9ec2937b55dd7eca692b7b0405188b066952aaef5eb00c316c502c00627e` |
| `checkpoint_210min_runtime_state.txt` | 3,943 bytes | `913b30cf3b9e06599f7f3085913efc5f78e2320a74df6ac64d05d7567a45898d` |
| `checkpoint_210min_validation.json` | 3,987 bytes | `f25d9ff118639e452f2afa1f8988b9c108a43dcc67b3974f646ee1504d6c96e2` |
