...
### 2.273 ea8 fixed256 v5 20分钟精度节点

2.272实时记录已由主仓提交`660524fe39ec448643f36d333e58b551980e7af3`通过GitHub HTTPS
发布，实验容器和tmux monitor在发布期间未中断。第二个固定节点由sidecar于
2026-08-03T13:05:44Z写出：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 7/256 | 6 | 85.714286% | 2.343750% | 0 | 0 |

相对10分钟节点新增2题，2题均正确；累计错误仍为1题。独立validator按20分钟UTC截止时间筛选
checkpoint文件，复算仍为7题完成、6题正确、0 invalid、0 truncated，与sidecar逐项一致。当前
样本仍不足以证明终局精度达到BF16基线，正式门禁继续保持至少105/256。

13:06:03Z运行快照显示容器与tmux monitor均存活，8卡各76,133 MiB，利用率97%–100%；最近
三轮服务指标为16 running/0 waiting、48.0–60.8 token/s、KV cache 7.9%–8.1%。日志扫描未发现
Traceback、fatal、OOM或RuntimeError。独立checkpoint validator 12/12 passed，immutable
manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_20min.log` | 146 bytes | `9e3b4e4e4e49f4f9b5b407c79723c468f87eafaa3a236425b9ed03d99d48c102` |
| `checkpoint_20min_gpu.csv` | 108 bytes | `7d122c9f138f9d78255fa9728bd5b9ef164774d9ce5da24f909de322636a3e29` |
| `checkpoint_20min_engine.log` | 711 bytes | `1930e3af0ef97e9569333cf6606b39fdec054856fe097322173b461fa3757dd3` |
| `checkpoint_20min_runtime_state.txt` | 147 bytes | `ccf63f47176d26dd9d2199ab14651c0aaa51f54c850217eb2ff336be4d78d8c5` |
| `checkpoint_20min_validation.json` | 1,541 bytes | `b2bb30f2dfff24735557a5b22e2937337cbd049c89fd5e66445487edded032fb` |
| `checkpoint_20min_manifest.sha256` | 1,703 bytes | `fb8e55038097e78e1d5742cc16e4d448062307f61c6111ac59bb9670f33d1f2d` |
| `checkpoint_20min_manifest_check.log` | 1,021 bytes | `79bbd7f63d0485c9215996c6b88fa0c5c4d99598ac51920eb4f15bb5be4e9f30` |

实验继续运行；30分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不因前7题的高当前精度提前改变候选实现。

### 2.274 ea8 fixed256 v5 30分钟精度节点

2.273实时记录已由主仓提交`632eb876ed3e48955ed59e37c8c4167b0a5fc207`通过GitHub HTTPS
发布，实验未中断。第三个固定节点由sidecar于2026-08-03T13:15:44Z写出；sidecar实际elapsed
为1,801秒：
...
相对20分钟节点没有新增完成题。首批其余请求仍在长输出生成，这一“无新增”节点不应解释为服务
停止。独立validator按30分钟UTC截止时间筛选checkpoint文件，复算仍为7题完成、6题正确、
0 invalid、0 truncated，与sidecar一致。

13:16:10Z运行快照显示容器与tmux monitor均存活，8卡各76,135 MiB，利用率97%–100%；最近
三轮服务指标为16 running/0 waiting、48.0–60.8 token/s、KV cache 13.5%–13.7%。日志扫描未
发现Traceback、fatal、OOM或RuntimeError。独立checkpoint validator 14/14 passed，immutable
manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_30min.log` | 146 bytes | `ec47671c0964fb8cabc96a68af91ffb28f6c7c56419dafe25b39e5ebdca37843` |
| `checkpoint_30min_gpu.csv` | 108 bytes | `b97bd79f50b097559984e3a42a05605644cbfabddb5515bedf76c5172fc6c4ff` |
| `checkpoint_30min_engine.log` | 714 bytes | `f6bbedc39ea70ad6dd47e3ddc9da1aaddbd084d3e46dbdae908e0325d1c35626` |
| `checkpoint_30min_runtime_state.txt` | 147 bytes | `d1ebbc3df8137cf625600bc158e848dc1461867a647327935e7bb5d20e8e729f` |
| `checkpoint_30min_validation.json` | 1,784 bytes | `0315577c05ac873462b3e1eefba0093abef0707f2991ee192714ae8c4f589836` |
| `checkpoint_30min_manifest.sha256` | 1,697 bytes | `c1a7433d468a0f81e113b1c733d0eb741580fac5400ffd7326b74c015b7d06e1` |
| `checkpoint_30min_manifest_check.log` | 1,015 bytes | `4bf10ff4f4a2e6eb6ec55603020ab26585b6337134e1927106b3ea88177c8a36` |
...
### 2.276 ea8 fixed256 v5 50分钟精度节点

2.275实时记录已由主仓提交`4d3f5c5f551bc21cf3300b3496fcd3b0be88dbb5`通过GitHub HTTPS
发布，实验未中断。第五个固定节点由sidecar于2026-08-03T13:35:44Z写出；实际elapsed为
3,001秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 35/256 | 17 | 48.571429% | 6.640625% | 0 | 16 |

相对40分钟节点新增28题，其中11题正确、16题截断；累计错误由1题增至18题，已完成题当前精度
从85.714286%回落至48.571429%。这说明前7题的高精度不可外推，但当前35题样本仍不足以证明
终局是否达到BF16的105/256门禁。

13:36:10Z运行快照显示容器与tmux monitor均存活，8卡各76,141 MiB，利用率76%–99%；新一批
16个请求为16 running/0 waiting，最近三轮生成吞吐76.8–78.4 token/s，KV cache 2.6%–2.9%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按50分钟UTC截止时间复算仍为35题完成、17题正确、0 invalid、16 truncated，
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_50min.log` | 149 bytes | `973eb2f73e5643c1e1d1f2b4dba5da8529f9bc420b520303954d00c1471e58a0` |
| `checkpoint_50min_gpu.csv` | 104 bytes | `097f46ecbe052ed7c6bf0dbeb6ca7c492530f500c2423f4c0563209fbd300caf` |
| `checkpoint_50min_engine.log` | 711 bytes | `729309a82b927dd512ee0298ec913b2eff1d8804ef24a2d1661223bc1d001a3b` |
| `checkpoint_50min_runtime_state.txt` | 147 bytes | `eaa196fa2ec391558622c0857e42d8abbd81547eb43a58af3eaaee5919b48aad` |
| `checkpoint_50min_validation.json` | 1,790 bytes | `5f269dd1e04b86e5207d5cf50435f696746aeebaa20e7cf53b28fd25ab11d551` |
| `checkpoint_50min_manifest.sha256` | 1,697 bytes | `5573791f7baff5f59e1d4d08e2b1bc25bcd474d1bb016eb4451338b4a2f7d052` |
| `checkpoint_50min_manifest_check.log` | 1,015 bytes | `a1501338044ca8a3648dd09b835efce93ffa0aa8b7a695477ee15361a2450e6a` |

实验继续运行；60分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不根据当前35题结果提前改变候选。

### 2.277 ea8 fixed256 v5 60分钟精度节点

2.276实时记录已由主仓提交`46286eb0285b64696cb4bb9ecd1a5edd243bc112`通过GitHub HTTPS
发布，实验未中断。第六个固定节点由sidecar于2026-08-03T13:45:44Z写出；实际elapsed为
3,601秒：
...
相对50分钟节点新增2题，其中1题正确、0题新增截断；已完成题当前精度基本持平。当前37题样本仍
不能证明终局达到BF16的105/256门禁。

13:46:10Z运行快照显示容器与tmux monitor均存活，8卡各76,141 MiB，利用率97%–100%；当前
16 running/0 waiting，最近三轮生成吞吐59.2–60.8 token/s，KV cache 9.3%–9.5%。日志扫描
未发现Traceback、fatal、OOM或RuntimeError。

独立validator按60分钟UTC截止时间复算仍为37题完成、18题正确、0 invalid、16 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_60min.log` | 149 bytes | `5771fe8dffe27e0d3c237dbdfeb12f2cb43f20c07fa3511f1442e96765148d00` |
| `checkpoint_60min_gpu.csv` | 106 bytes | `5183f8c6b8fc85b63b6f349a83ef3c17129dc9a599feec0a2f83515e9f17e841` |
| `checkpoint_60min_engine.log` | 711 bytes | `93fe193d91f543524f922a344c52c27386cbae23239744946fd289dd87548f74` |
| `checkpoint_60min_runtime_state.txt` | 150 bytes | `659012bc44c68c662f203276d821e085ad0363b25dc21832803d2dd023fafea3` |
| `checkpoint_60min_validation.json` | 1,790 bytes | `32f295caa1b13b64d0a6cb1195ca385e7ec44b45cf1b667891aa55640382637d` |
| `checkpoint_60min_manifest.sha256` | 1,697 bytes | `6c1e77e4d7d438ef19eaa9ee9e5a93134caf442835f7fb457c30966d2953a1c0` |
| `checkpoint_60min_manifest_check.log` | 1,015 bytes | `e2562780596b2cf59934c847aa4cb75a0ecdffc30eb143eadbcf86e618320e36` |
...
### 2.280 ea8 fixed256 v5 90分钟精度节点

2.279实时记录已由主仓提交`edb5e97a6c2a8c1c34bfea9c0bc4e00066203c10`通过GitHub HTTPS
发布，实验未中断。第九个固定节点由sidecar于2026-08-03T14:15:44Z写出；实际elapsed为
5,401秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 71/256 | 34 | 47.887324% | 13.281250% | 0 | 32 |

相对80分钟节点新增20题，其中10题正确、9题新增截断；已完成题当前精度从47.058824%
升至47.887324%。当前71题样本仍不能证明终局是否达到BF16的105/256门禁。

14:16:03Z运行快照显示容器与tmux monitor均存活；同时的8卡各76,143 MiB，
利用率98%–100%。最近三轮服务指标为16 running/0 waiting，生成吞吐64.0–65.6 token/s，
KV cache 6.0%–6.3%。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按90分钟UTC截止时间复算为71题完成、34题正确、0 invalid、32 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_90min.log` | 150 bytes | `d64c34a22db42506a225bb6401030d52e8105e99570a139fa5f0973b1aa4de42` |
| `checkpoint_90min_gpu.csv` | 523 bytes | `7f84ba4b638afe1335cce93767b86bb92a0ff92c3e44605d805c80fd1a7350c9` |
| `checkpoint_90min_engine.log` | 711 bytes | `7208cb0497c25a248f0a2288a2f01d662deaffb6a76f4931089ba5015ef1a24c` |
| `checkpoint_90min_runtime_state.txt` | 144 bytes | `cb12157dbfe1fde55cf123c62812336ec205bf516d8dc5212b6b080f47cf89db` |
| `checkpoint_90min_validation.json` | 1,790 bytes | `80b172dd7490af1f7922df1983c8997e264443c184d49df03a2f65bc48fd0d4e` |
| `checkpoint_90min_manifest.sha256` | 1,697 bytes | `2471090dec3dc4c8a9e4cba4bde4ea3013a04f2ff80861d91c4c65effb24fdaa` |
| `checkpoint_90min_manifest_check.log` | 1,015 bytes | `9e6ee161bbe2671e24867394cf6ff714e36d0cbbbb17ceb74d3f39eaf2539cf1` |

实验继续运行；100分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不根据当前71题结果提前改变候选。

### 2.281 ea8 fixed256 v5 100分钟精度节点

2.280实时记录已由主仓提交`bfc21da2e14a29d650a2316d2510452beb8ed22f`通过GitHub HTTPS
发布，实验未中断。第十个固定节点由sidecar于2026-08-03T14:25:44Z写出；实际elapsed为
6,001秒：
...
相对90分钟节点无新增完成题，但这不是服务停止。14:26:02Z快照显示容器与
tmux monitor均存活，8卡各76,143 MiB、利用率97%–100%；最近三轮服务指标为
16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 12.3%–12.4%，说明当前长输出仍
持续生成。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按100分钟UTC截止时间复算为71题完成、34题正确、0 invalid、32 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_100min.log` | 150 bytes | `d388cc8dab1827e0b42b3eea6c4b445322960d70ec2ed6c7ca7c361ae2e83d6e` |
| `checkpoint_100min_gpu.csv` | 523 bytes | `6291936a996d899dd45eac9656584ace6a68076ed1419f9bde136e475611cd72` |
| `checkpoint_100min_engine.log` | 714 bytes | `1af4550e3e72b294afb46acf4f236b470a5345d405f515da6364b19567151cbe` |
| `checkpoint_100min_runtime_state.txt` | 144 bytes | `e5369239e8163ce912092cfad4b3f17fda599fafe6173b3699e87223f613944b` |
| `checkpoint_100min_validation.json` | 1,790 bytes | `626bcedfae541350264453b9e6718f7f3e33a0f1eceef663d2b8293a6808df78` |
| `checkpoint_100min_manifest.sha256` | 1,704 bytes | `ed016634cd79e733b02c296de80875eacc21a0cbc07be9966c3f4052925e05db` |
| `checkpoint_100min_manifest_check.log` | 1,022 bytes | `67f6ea966b1b3d2918e56da9bb019bb30db076b5cf7e71e053b668129077e46c` |

实验继续运行；110分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不因100分钟无新增而重启候选。

### 2.282 ea8 fixed256 v5 110分钟精度节点

2.281实时记录已由主仓提交`c223f964cc2080322701c8a15bc132b5f01414d0`通过GitHub HTTPS
发布，实验未中断。第十一个固定节点由sidecar于2026-08-03T14:35:44Z写出；实际elapsed为
6,601秒：
...
相对100分钟节点仍无新增完成题，但连续两个无新增节点仍不是服务停止。14:36:02Z
快照显示容器与tmux monitor均存活，8卡各76,143 MiB、利用率97%–98%；最近三轮服务
指标为16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 18.3%–18.5%，说明当前长输出
仍持续生成。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按110分钟UTC截止时间复算为71题完成、34题正确、0 invalid、32 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_110min.log` | 150 bytes | `9f2e732a56d5879124d3cf237a7ebb0a27b63986b560bc3bd17aa5a319811d76` |
| `checkpoint_110min_gpu.csv` | 520 bytes | `434c571b418241c7b0c3ed2d42368ff835aa7da0218aa29d291f1b53c9cbc71f` |
| `checkpoint_110min_engine.log` | 714 bytes | `22cce4a4161d05331a3e7e73ecf6e4c4cc946d068360d9be2a030b3a6fdc8ed8` |
| `checkpoint_110min_runtime_state.txt` | 144 bytes | `c4999d6984e89f884c37b2b12e053464b20e06b10d00e04160850340ed25c1de` |
| `checkpoint_110min_validation.json` | 1,790 bytes | `0de83b6b45fdbc0405143cd7a4a5cd5c34b9abdd4f3135ad91f58faffa93d37b` |
| `checkpoint_110min_manifest.sha256` | 1,704 bytes | `74becb67c669564a68a17c14568ab6a761d3f8da530b6d3fc69f77b0b88eb89a` |
| `checkpoint_110min_manifest_check.log` | 1,022 bytes | `3331feddfb5548039347d1784ba3b3492da01f036bdf80acd5fa6763bd4a12e5` |

实验继续运行；120分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不因连续两个节点无新增而重启候选。

### 2.283 ea8 fixed256 v5 120分钟精度节点

2.282实时记录已由主仓提交`8957feb5d4fd8ef3800abd24359798b7195fb3ea`通过GitHub HTTPS
发布，实验未中断。第十二个固定节点由sidecar于2026-08-03T14:45:44Z写出；实际elapsed为
7,201秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 101/256 | 45 | 44.554455% | 17.578125% | 0 | 47 |

相对110分钟节点新增30题，其中11题正确、15题新增截断；已完成题当前精度从47.887324%
降至44.554455%。当前101题样本仍不能证明终局是否达到BF16的105/256门禁。

14:46:01Z运行快照显示容器与tmux monitor均存活，8卡各76,143 MiB、利用率86%–98%；
最近三轮服务指标为15–16 running/0 waiting，生成吞吐71.9–76.8 token/s，KV cache 4.0%–4.1%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按120分钟UTC截止时间复算为101题完成、45题正确、0 invalid、47 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_120min.log` | 151 bytes | `6cd6bc54980a77edf38c62684d6534188cb4354d85ccf56bf64b24f0267ddc91` |
| `checkpoint_120min_gpu.csv` | 520 bytes | `ad1c780710dbe9df9d6cecf54a60700e6555bc978de7c43811e17220e566d6cf` |
| `checkpoint_120min_engine.log` | 711 bytes | `18ae0f124c52f1b178ff431d695f2513285130156e59a6798e8e29c874e4ab26` |
| `checkpoint_120min_runtime_state.txt` | 144 bytes | `10d0a3321e96c63df23bc83a36796e5995c72466215044b2d832bb8c0728e4a5` |
| `checkpoint_120min_validation.json` | 1,792 bytes | `7ffe669bf3ddb72538e128474a0a39e286db57e970758754675194848d79ece6` |
| `checkpoint_120min_manifest.sha256` | 1,704 bytes | `af95a75355cda7a0a2b9777ffd499901da1028fa1c85acc396f6fa2ad3b9599c` |
| `checkpoint_120min_manifest_check.log` | 1,022 bytes | `53ae5944d6cd1b8597b777fe9a822773e0fb251ab44747d31a1cbbf2edd6e998` |

实验继续运行；130分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不根据当前101题结果提前改变候选。

### 2.284 ea8 fixed256 v5 130分钟精度节点

2.283实时记录已由主仓提交`dad7093e945771e0dbc43e456734f9bf6a6498c0`通过GitHub HTTPS
发布，实验未中断。第十三个固定节点由sidecar于2026-08-03T14:55:44Z写出；实际elapsed为
7,801秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 105/256 | 48 | 45.714286% | 18.750000% | 0 | 48 |

相对120分钟节点新增4题，其中3题正确、1题新增截断；已完成题当前精度从44.554455%
升至45.714286%。“已完成105题”不等于达到BF16门禁：门禁比较的是最终正确数至少105/256，当前
正确数只有48，必须继续自然完成。

14:56:02Z运行快照显示容器与tmux monitor均存活，8卡各76,143 MiB、利用率97%–100%；
最近三轮服务指标为16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 9.3%–9.5%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按130分钟UTC截止时间复算为105题完成、48题正确、0 invalid、48 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_130min.log` | 151 bytes | `72da6995194e5551400f73d8a308552f13df31f83425ae82b0eeccd6d8d24b47` |
| `checkpoint_130min_gpu.csv` | 521 bytes | `a09a15e31d7eb9eb6cdecfc684121bfd57a2846791422b5491b01b291e7ec399` |
| `checkpoint_130min_engine.log` | 711 bytes | `2e3b5fa3b29420b9a6c1e048b5aa23355cc9c2cc37a8c69f7321ec3600b9f1de` |
| `checkpoint_130min_runtime_state.txt` | 144 bytes | `0ce62d691d6d64c75cf8bc9d3ae3211c258a47ef1ebef0e36d39323a5717d644` |
| `checkpoint_130min_validation.json` | 1,792 bytes | `2440a9189b56ee21d1a9c8acf539c06000ca7832dc160dce58c9b66539a91c61` |
| `checkpoint_130min_manifest.sha256` | 1,704 bytes | `e26702e154324de1f5a398ca55f67835fcd9ea175b931c47552a1390047ccd25` |
| `checkpoint_130min_manifest_check.log` | 1,022 bytes | `38e2e66962910e3df982bba400e8eeef4e516e986831d7a940d811f4dc4956fe` |

实验继续运行；140分钟节点继续使用同一固定口径。达到BF16 105/256前禁止启动32K/batch1
性能复测，也不将已完成题数误解为正确数门禁。

### 2.285 ea8 fixed256 v5 140分钟精度节点

2.284实时记录已由主仓提交`42f6aa7015b864d42a953e7cb587f0caee05f163`通过GitHub HTTPS
发布，实验未中断。第十四个固定节点由sidecar于2026-08-03T15:05:44Z写出；实际elapsed为
8,401秒：
...
相对130分钟节点无新增完成题，但这不是服务停止。15:05:59Z快照显示容器与
tmux monitor均存活，8卡各76,143 MiB、利用率97%–100%；最近三轮服务指标为
16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 15.4%–15.5%，说明当前长输出仍
持续生成。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按140分钟UTC截止时间复算为105题完成、48题正确、0 invalid、48 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_140min.log` | 151 bytes | `c9b4ce4d49673df1fda7167e8fe4d3c1e450f11bb814a29928f2fe4ba9ac85d6` |
| `checkpoint_140min_gpu.csv` | 522 bytes | `aeb8c09e3feae06a0225287d0228213a64bf16ff531be347026f0edacdeb1701` |
| `checkpoint_140min_engine.log` | 714 bytes | `b14b77595472439414c91fa06051d04f794202ba075c87aeca332d1789a0bc48` |
| `checkpoint_140min_runtime_state.txt` | 144 bytes | `dff3089db34364b14a63e1ad247dc2cc359e026c6efb8e551e974a81ed40df33` |
| `checkpoint_140min_validation.json` | 1,792 bytes | `c2f78d6c42b91fb400bc7e8b94eac28f5ead8bb945c9cd66aec85a53d4f6dd19` |
| `checkpoint_140min_manifest.sha256` | 1,704 bytes | `a200ff40f8d79e7ccf905b852b484cc3a622179f48e065ceb03071391b115845` |
| `checkpoint_140min_manifest_check.log` | 1,022 bytes | `cd5cdc783b872c169957d7c098208cf56a3a1ea4296fd1c86f98b3891775d66c` |

实验继续运行；150分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不因140分钟无新增而重启候选。

### 2.286 ea8 fixed256 v5 150分钟精度节点

2.285实时记录已由主仓提交`fd07a36c6e7f7791c3290992c2603be4487d5580`通过GitHub HTTPS
发布，实验未中断。第十五个固定节点由sidecar于2026-08-03T15:15:45Z写出；实际elapsed为
9,002秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 113/256 | 49 | 43.362832% | 19.140625% | 0 | 56 |

相对140分钟节点新增8题，其中1题正确、8题新增截断；已完成题当前精度从45.714286%
降至43.362832%。当前113题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

15:16:02Z运行快照显示容器与tmux monitor均存活，8卡各76,145 MiB、利用率98%–99%；
最近三轮服务指标为15–16 running/0 waiting，生成吞吐65.5–76.8 token/s，KV cache 10.9%–11.1%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按150分钟UTC截止时间复算为113题完成、49题正确、0 invalid、56 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_150min.log` | 151 bytes | `d64cddc43008f28dfb6244d95e663f51bbbcd1313a366232cdcc1079541113a9` |
| `checkpoint_150min_gpu.csv` | 520 bytes | `7a4df7e4c5c201a816421f5d1dfef788ad3acb68a6c3583d0df797f4a44c364a` |
| `checkpoint_150min_engine.log` | 714 bytes | `62babc1a8e409ed76770a47c11f158c983f180e6fa5858195f65bbb668e44963` |
| `checkpoint_150min_runtime_state.txt` | 144 bytes | `c3df5d4a3fa44d217fd1037a4751749c24c3b3deb71f4da36de5c9bd4ac8c110` |
| `checkpoint_150min_validation.json` | 1,792 bytes | `7a3857bd1fc5a3263f90dbf1cecf7264464a77a0ae14dbcd6aef9634f2b5fd5c` |
| `checkpoint_150min_manifest.sha256` | 1,704 bytes | `32259a56f6160cd007badd4e21aa21e20bc6e20f1eae84aea3f3c3ab2a323e2e` |
| `checkpoint_150min_manifest_check.log` | 1,022 bytes | `4ec131715c6a00b9dd37df67413402dd66e47ce8e80a03a2344e4296eae76a90` |

实验继续运行；160分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前113题结果提前改变候选。

### 2.287 ea8 fixed256 v5 160分钟精度节点

2.286实时记录已由主仓提交`f212998a2c34f8e03f9753d49e3a3e11d5e557f1`通过GitHub HTTPS
发布，实验未中断。第十六个固定节点由sidecar于2026-08-03T15:25:45Z写出；实际elapsed为
9,602秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 127/256 | 54 | 42.519685% | 21.093750% | 0 | 64 |

相对150分钟节点新增14题，其中5题正确、8题新增截断；已完成题当前精度从43.362832%
降至42.519685%。当前127题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

15:26:43Z运行快照显示容器与tmux monitor均存活，8卡各76,145 MiB、利用率97%–100%；
最近三轮服务指标为16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 7.9%–8.1%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按160分钟UTC截止时间复算为127题完成、54题正确、0 invalid、64 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_160min.log` | 151 bytes | `8d66b4c976199d942a3d4c39771787f9bc93f9b21829ce6377114eaa118a56a6` |
| `checkpoint_160min_gpu.csv` | 520 bytes | `b7995a4bf360327e533748d8275b3236802c980a3acb2795825a2292e032a40b` |
| `checkpoint_160min_engine.log` | 711 bytes | `41433898a4dc89776e9bafa8567c21426a5aa80344ee1c2a2b33337baedba162` |
| `checkpoint_160min_runtime_state.txt` | 144 bytes | `35bc2bc43be4c18e4694ffa7f709500625dca3bcc01cee36d038541dc35c72b2` |
| `checkpoint_160min_validation.json` | 1,792 bytes | `384865d3e57ec75d51da3900dd5f5c9b94a2a1a3669ec30631096916ed27016d` |
| `checkpoint_160min_manifest.sha256` | 1,704 bytes | `0a1f26a1b41e532657ec4d2e37891cb2351f4b4edc8f401850fcfe19b03665bb` |
| `checkpoint_160min_manifest_check.log` | 1,022 bytes | `723f502a60f6312ddfbdf21130ecc7bb34512576dea4c677cb54105f940cd1eb` |

实验继续运行；170分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前127题结果提前改变候选。

### 2.288 ea8 fixed256 v5 170分钟精度节点

2.287实时记录已由主仓提交`57526a36aa4575eb7b08a855079050720810e68b`通过GitHub HTTPS
发布，实验未中断。第十七个固定节点由sidecar于2026-08-03T15:35:45Z写出；实际elapsed为
10,202秒：
...
相对160分钟节点无新增完成题，但这不是服务停止。15:36:16Z快照显示容器与
tmux monitor均存活，8卡各76,145 MiB、利用率98%–100%；最近三轮服务指标为
16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 13.2%–13.4%，说明当前长输出仍
持续生成。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按170分钟UTC截止时间复算为127题完成、54题正确、0 invalid、64 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_170min.log` | 152 bytes | `71dcebb57fc7ebf38aab5a8b19b9b7ef9ad8bf83ac06d8c80336464c0b231293` |
| `checkpoint_170min_gpu.csv` | 522 bytes | `1a596245f206738ebe2abad285c9d792811a318a7278c3db020a565ebb59276d` |
| `checkpoint_170min_engine.log` | 714 bytes | `abecea1716ecc59322d2794383cb6f7baad551a60f42c0c1ef295e389713e82e` |
| `checkpoint_170min_runtime_state.txt` | 144 bytes | `d794584802e8e7dd885ef77a5bcfc9b82b4cea4b44998feac4364ceec574da06` |
| `checkpoint_170min_validation.json` | 1,792 bytes | `9abab9a18dfcd7231bd7b5e094d35d343428383e7544d684e8e7d61dfe3933df` |
| `checkpoint_170min_manifest.sha256` | 1,704 bytes | `9795a68e29f589f8d23d68ea5e7c161c39064d477fe8fa2d5d5d5bc9200d0b0b` |
| `checkpoint_170min_manifest_check.log` | 1,022 bytes | `cfb7ab5f38bbed442327a03bf4fac76115bf80f54e5d4196ac0042a298f0d97a` |

实验继续运行；180分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不因170分钟无新增而重启候选。

### 2.289 ea8 fixed256 v5 180分钟精度节点

2.288实时记录已由主仓提交`247b5848c78dd837b3c0f26143499cb8c04631e8`通过GitHub HTTPS
发布，实验未中断。第十八个固定节点由sidecar于2026-08-03T15:45:45Z写出；实际elapsed为
10,802秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 131/256 | 54 | 41.221374% | 21.093750% | 0 | 67 |

相对170分钟节点新增4题、无新增正确、3题新增截断；已完成题当前精度从42.519685%
降至41.221374%。当前131题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

15:46:18Z运行快照显示容器与tmux monitor均存活，8卡各76,145 MiB；该单点GPU利用率
恰为0%，不能单独据此判定服务停止。此前10–30秒内最近三轮服务指标均为16 running/0 waiting，
生成吞吐55.9–65.6 token/s、KV cache 15.2%–15.4%，与当前批次继续生成一致。日志扫描未发现
Traceback、fatal、OOM或RuntimeError。

独立validator按180分钟UTC截止时间复算为131题完成、54题正确、0 invalid、67 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_180min.log` | 152 bytes | `792fa965cec14738d5fd179935df843d701d7bb964ddad5eab2d2291fe590220` |
| `checkpoint_180min_gpu.csv` | 510 bytes | `c47e1d1ea92ea19b69c027aa54d809c3e3f7a61d9ee6806363e8252a9a5afd1a` |
| `checkpoint_180min_engine.log` | 714 bytes | `a98fb47f1375c9c4f3627ee2ab0b6c670dc2b23c52b64e747e56c767a3064074` |
| `checkpoint_180min_runtime_state.txt` | 144 bytes | `784705525987661ae4ed1b96b231e287edb6e7486a724e1c6783aa708fe0ca4c` |
| `checkpoint_180min_validation.json` | 1,792 bytes | `0dfa6120463a09f43f655cb1fd72a4f2c01b1f9c2dbb998015948b8e1fa5938b` |
| `checkpoint_180min_manifest.sha256` | 1,704 bytes | `45e2a4e74193b4cb8890f800fdc9e7e2b22558725bddb30fa7ad743939c1da4d` |
| `checkpoint_180min_manifest_check.log` | 1,022 bytes | `d7bd4e88f8930d737ed43ec28b0c227217d488e4d403dfa7826ffdc13e18969b` |

实验继续运行；190分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前131题结果提前改变候选。

### 2.290 ea8 fixed256 v5 190分钟精度节点

2.289实时记录已由主仓提交`f4f1df41161af22de36a6a2152b047c77a1b5585`通过GitHub HTTPS
发布，实验未中断。第十九个固定节点由sidecar于2026-08-03T15:55:45Z写出；实际elapsed为
11,402秒：
...
相对180分钟节点新增23题，其中10题正确、13题新增截断；已完成题当前精度从41.221374%
升至41.558442%。当前154题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

15:56:13Z运行快照显示容器与tmux monitor均存活，8卡各76,145 MiB、利用率均为98%；
最近三轮服务指标为16 running/0 waiting，生成吞吐76.8–78.4 token/s，KV cache 4.2%–4.4%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按190分钟UTC截止时间复算为154题完成、64题正确、0 invalid、80 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_190min.log` | 152 bytes | `7e8547b8ca7f12c76b3875cf6fabbe58d14af55237a5c83d7089d5c85b341727` |
| `checkpoint_190min_gpu.csv` | 520 bytes | `b764eaa274df264864600e38b0ef4432a1d3bad88289c4ec4c817f45abf13477` |
| `checkpoint_190min_engine.log` | 711 bytes | `bdf1a9d880edc9b983a6a6dadafa18d1baa913fabb9b5ee33cad0e5e9d710a39` |
| `checkpoint_190min_runtime_state.txt` | 144 bytes | `13d55a3deba4382380abf06d853ed2461ea3b56aa08a7e01feb3f680aa1d4765` |
| `checkpoint_190min_validation.json` | 1,792 bytes | `623fb23bb0352f03eec4d2f09327150007ff0f3ab12e9fd2b75a5209c7cdda9a` |
| `checkpoint_190min_manifest.sha256` | 1,704 bytes | `3cc9aabdb502c3d7f5a99aa01fba2de64ae154496c39d2d53a6cd80efc9f58d3` |
| `checkpoint_190min_manifest_check.log` | 1,022 bytes | `660161025337a3902045b9f9081c88c9dd8fbe0346977f1be2a680398e3fde37` |
...
### 2.292 ea8 fixed256 v5 210分钟精度节点

2.291实时记录已由主仓提交`7f40d7bb4533614c69df01dabad94be84df0b9f7`通过GitHub HTTPS
发布，实验未中断。第二十一个固定节点由sidecar于2026-08-03T16:15:45Z写出；实际elapsed为
12,602秒：
...
相对200分钟节点仍无新增完成题，但连续两个无新增节点仍不是服务停止。16:16:12Z
快照显示容器与tmux monitor均存活，8卡各76,145 MiB、利用率98%–100%；最近三轮服务
指标为16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 16.7%–16.9%，说明当前长输出
仍持续生成。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按210分钟UTC截止时间复算为154题完成、64题正确、0 invalid、80 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_210min.log` | 152 bytes | `5d4a74ca2f920a609c2b40ec1d465f008ee7a5760e37f7da4373b3fdcc4b18eb` |
| `checkpoint_210min_gpu.csv` | 523 bytes | `e8719650f0f23defc932e057e7fa5209bf457a32d0e714358dcb63cb79d6c782` |
| `checkpoint_210min_engine.log` | 714 bytes | `3636dbd35f47c0cb1a9f542f810321d4c59fae09068aef1505f269442094090c` |
| `checkpoint_210min_runtime_state.txt` | 144 bytes | `d4b12f3dc867217009a47d6aa7c71b67844ad38c6e36a0a9058f7b32ea6900e3` |
| `checkpoint_210min_validation.json` | 1,792 bytes | `d9273bf50edc0131d43b5e4265006dc13a3e61944bcff9925dcb8216e2240f9e` |
| `checkpoint_210min_manifest.sha256` | 1,704 bytes | `aa290758fb15a95f6a82a697789d60e5ab2a0acbe65fbfcfc0cb8bae195680b2` |
| `checkpoint_210min_manifest_check.log` | 1,022 bytes | `5cd55efa24ed7e530f8fbe176417428db47b79c256b1b808cee93f0279d4133e` |

实验继续运行；220分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不因连续两个节点无新增而重启候选。

### 2.293 ea8 fixed256 v5 220分钟精度节点

2.292实时记录已由主仓提交`dd54e3a5184ecee38d31c0a9eeea358a4bed1949`通过GitHub HTTPS
发布，实验未中断。第二十二个固定节点由sidecar于2026-08-03T16:25:45Z写出；实际elapsed为
13,202秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 171/256 | 69 | 40.350877% | 26.953125% | 0 | 90 |

相对210分钟节点新增17题，其中5题正确、10题新增截断；已完成题当前精度从41.558442%
降至40.350877%。当前171题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

16:26:14Z运行快照显示容器与tmux monitor均存活，8卡各76,147 MiB、利用率84%–98%；
最近三轮服务指标为16 running/0 waiting，生成吞吐60.6–76.7 token/s，KV cache 6.8%–9.4%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按220分钟UTC截止时间复算为171题完成、69题正确、0 invalid、90 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_220min.log` | 152 bytes | `59b41d023d631c2f07153a4810f5cbe0cb7d11b1a682abaa32d1d4aa6abe57c7` |
| `checkpoint_220min_gpu.csv` | 520 bytes | `d02b307a624580226d9c083ea46ae160243c551209ba9e3d189ef35eb58e652a` |
| `checkpoint_220min_engine.log` | 714 bytes | `823b9f9a3f427a232f37b4deaa73c129dbaeb6d6166b1eeca3df93e49f0f4d93` |
| `checkpoint_220min_runtime_state.txt` | 144 bytes | `d00bbc539f2f2b6a6ffe0cd3e73c6efa9b164fd7dde09e05d49a203a8dabd646` |
| `checkpoint_220min_validation.json` | 1,792 bytes | `bf7eb26ac601f0e3c3fad3c4fbf19790458c6aaabc1f2e33c7980fb75b504145` |
| `checkpoint_220min_manifest.sha256` | 1,704 bytes | `0b66816e0ad6d424414f069ac726920de236f677c74da8d43f10cd2001262230` |
| `checkpoint_220min_manifest_check.log` | 1,022 bytes | `d705850ba2ac53142cf62e644128b2e2bc2d83a20fc17d1aec2ac87fb3dbcbfc` |

实验继续运行；230分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前171题结果提前改变候选。

### 2.294 ea8 fixed256 v5 230分钟精度节点

2.293实时记录已由主仓提交`7ae8b1a7ac61f74b68d7898a6553a1a311077184`通过GitHub HTTPS
发布，实验未中断。第二十三个固定节点由sidecar于2026-08-03T16:35:45Z写出；实际elapsed为
13,802秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 185/256 | 76 | 41.081081% | 29.687500% | 0 | 96 |

相对220分钟节点新增14题，其中7题正确、6题新增截断；已完成题当前精度从40.350877%
升至41.081081%。当前185题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

16:36:14Z运行快照显示容器与tmux monitor均存活，8卡各76,147 MiB、利用率97%–100%；
最近三轮服务指标为16 running/0 waiting，生成吞吐均为62.4 token/s，KV cache 7.7%–7.9%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按230分钟UTC截止时间复算为185题完成、76题正确、0 invalid、96 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_230min.log` | 152 bytes | `32c464c4ab10e2d873a2bb6755c9b6a9cc4ede9f4756328736cd8e63fd4d6620` |
| `checkpoint_230min_gpu.csv` | 521 bytes | `1087874922620cbcf9e5b8be97b4ad3334403c481f24ae267a22835f7738eea7` |
| `checkpoint_230min_engine.log` | 711 bytes | `6da2cc7cc7021d32ea8158a51ea8e3a136eca99e1f60444ebc3b2a4f9a8cfe8e` |
| `checkpoint_230min_runtime_state.txt` | 144 bytes | `5574eeb3f1befc1e84008e8e0a8cb05decc0cd9212f7c33b2cbba8b0a5ab07f3` |
| `checkpoint_230min_validation.json` | 1,792 bytes | `4613ec8262db8dbab4d89496c362a27034724ab70dbc135b7d3d3d110b21a891` |
| `checkpoint_230min_manifest.sha256` | 1,704 bytes | `81f934f16ebfdeb746d83875fa21d1698b2177097a47e71e96df869dd84b6ba5` |
| `checkpoint_230min_manifest_check.log` | 1,022 bytes | `6d82e1438beb7deee3748910436f56bf74926c9159b74ce31821fd59ea26abbd` |

实验继续运行；240分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前185题结果提前改变候选。

### 2.295 ea8 fixed256 v5 240分钟精度节点

2.294实时记录已由主仓提交`379a0b7af74e514d6a1fde59037bc78091f995ca`通过GitHub HTTPS
发布，实验未中断。第二十四个固定节点由sidecar于2026-08-03T16:45:45Z写出；实际elapsed为
14,402秒：
...
相对230分钟节点无新增完成题，但这不是服务停止。16:46:13Z快照显示容器与
tmux monitor均存活，8卡各76,147 MiB、利用率98%–100%；最近三轮服务指标为
16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 13.8%–14.0%，说明当前长输出仍
持续生成。日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按240分钟UTC截止时间复算为185题完成、76题正确、0 invalid、96 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_240min.log` | 152 bytes | `ee71263845972e5fde2ca034dd3858a9985e6b5f02d93245a9e02d75f53e7566` |
| `checkpoint_240min_gpu.csv` | 521 bytes | `54fcd5cab0196b88ce9b335e8ba24b68112524e05e2ebc419f0c0a10456b295c` |
| `checkpoint_240min_engine.log` | 714 bytes | `1cf7af69365f738b213e1ea7f67ae0f037dc6ff41e29c95f7cf6846297086215` |
| `checkpoint_240min_runtime_state.txt` | 144 bytes | `1013cbf9a25a064e631cdfbc051ea3116f0e9312f2664d80566d98f1d0e36cb1` |
| `checkpoint_240min_validation.json` | 1,792 bytes | `7a7d58eeb269520e1f46914331b0f190b2d412fae0d4ab59e1b35e2f89cad420` |
| `checkpoint_240min_manifest.sha256` | 1,704 bytes | `4e9fe8b13c2edabb2abb18df61b7a372c2f797ada432b8e5880ae7099795aa90` |
| `checkpoint_240min_manifest_check.log` | 1,022 bytes | `4959a941e0249c5c8f5b3f4c3c0e2d5872cd7ef200c7b165529783612d56afd2` |

实验继续运行；250分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不因240分钟无新增而重启候选。

### 2.296 ea8 fixed256 v5 250分钟精度节点

2.295实时记录已由主仓提交`993eeb4aa5d685ea341c9b1408e16bd8024b56a9`通过GitHub HTTPS
发布，实验未中断。第二十五个固定节点由sidecar于2026-08-03T16:55:45Z写出；实际elapsed为
15,002秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 189/256 | 78 | 41.269841% | 30.468750% | 0 | 99 |

相对240分钟节点新增4题，其中2题正确、3题新增截断；已完成题当前精度从41.081081%
升至41.269841%。当前189题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

16:56:13Z运行快照显示容器与tmux monitor均存活，8卡各76,147 MiB、利用率98%–100%；
最近三轮服务指标为16 running/0 waiting，生成吞吐64.0–65.6 token/s，KV cache 15.9%–16.1%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按250分钟UTC截止时间复算为189题完成、78题正确、0 invalid、99 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_250min.log` | 152 bytes | `fe7cac2a25dadff57a0b3d50758a19d890a9a81dbc2cd282857bee21fc27bcd8` |
| `checkpoint_250min_gpu.csv` | 522 bytes | `3dfafa44bbdedc9e62e2cab38cf2eabc58e0f5447690b9960058c369967c549b` |
| `checkpoint_250min_engine.log` | 714 bytes | `736ea806f2c50aa12ff8df5bb20714a44530c4ea52aabd5a31744f4312d33070` |
| `checkpoint_250min_runtime_state.txt` | 144 bytes | `bcbf7c5c6ec9377656ccb093b39b4a6a45ea0a35fdff9b5d8ded3a83635b42f8` |
| `checkpoint_250min_validation.json` | 1,792 bytes | `a43ed637f03f4240ad77e7342014e7eb9a21544b17928f1669a8a3c8b255bc4d` |
| `checkpoint_250min_manifest.sha256` | 1,704 bytes | `16721ed6fe53c77eb123fee678043dbaabce58ea001cee9fff822911cbacd5cf` |
| `checkpoint_250min_manifest_check.log` | 1,022 bytes | `6e5093ce04c8f5aa7e7ad28fa533cf5e451f9f52376a32295d1be685e5bed823` |

实验继续运行；260分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前189题结果提前改变候选。

### 2.297 ea8 fixed256 v5 260分钟精度节点

2.296实时记录已由主仓提交`fd0019a955c213e5c482294833504791ba932385`通过GitHub HTTPS
发布，实验未中断。第二十六个固定节点由sidecar于2026-08-03T17:05:46Z写出；实际elapsed为
15,603秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 219/256 | 91 | 41.552511% | 35.546875% | 0 | 112 |

相对250分钟节点新增30题，其中13题正确、13题新增截断；已完成题当前精度从41.269841%
升至41.552511%。当前219题样本仍不能证明终局是否达到BF16的105/256正确数门禁。

17:06:16Z运行快照显示容器与tmux monitor均存活，8卡各76,149 MiB、利用率95%–98%；
最近三轮服务指标为16 running/0 waiting，生成吞吐76.8–78.4 token/s，KV cache 4.2%–4.5%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按260分钟UTC截止时间复算为219题完成、91题正确、0 invalid、112 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_260min.log` | 153 bytes | `302d0d58efb8c460ba93389c136c65e9c0463aca37322497ce8ed40dd5546814` |
| `checkpoint_260min_gpu.csv` | 520 bytes | `4dd23ea89dca1e0fc04d8d617eda78729d96e947a5db006e1b568daf41d63ab7` |
| `checkpoint_260min_engine.log` | 711 bytes | `c4b75a28086137f6c638f6ed2f5e360170107cc45394bb24c96d222316df9c2f` |
| `checkpoint_260min_runtime_state.txt` | 144 bytes | `da65c3eefec358ebac878576ae112341bd8d3fda77e694e9414df3ec6e0bdbf2` |
| `checkpoint_260min_validation.json` | 1,794 bytes | `a7a3da325d93000e0e1e5724d5d2a7610886d4800b7c8a8fd81a10f79c2bb136` |
| `checkpoint_260min_manifest.sha256` | 1,704 bytes | `aa7b6fb6d37e29ff439e04f818391d3c582e8b3ef1ecc71b416d6359eeea33a2` |
| `checkpoint_260min_manifest_check.log` | 1,022 bytes | `c5beab996f5c3ab45b9ecf8247bcd9c13eb8a7527dca4c3b5a41572d5e5ae485` |

实验继续运行；270分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前219题结果提前改变候选。

### 2.298 ea8 fixed256 v5 270分钟精度节点

2.297实时记录已由主仓提交`2c778f323db5c516539f2dcd9a28a2b98a3b0c1b`通过GitHub HTTPS
发布，实验未中断。第二十七个固定节点由sidecar于2026-08-03T17:15:46Z写出；实际elapsed为
16,203秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 221/256 | 93 | 42.081448% | 36.328125% | 0 | 112 |

相对260分钟节点新增2题，2题均正确且无新增截断；已完成题当前精度从41.552511%
升至42.081448%。剩余35题至少还需12题正确才能达到BF16的105/256正确数门禁，当前221题
结果仍不能证明终局是否达标。

17:16:05Z运行快照显示容器与tmux monitor均存活，8卡各76,149 MiB、利用率97%–98%；
最近三轮服务指标为16 running/0 waiting，生成吞吐59.2–60.8 token/s，KV cache 10.7%–10.9%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按270分钟UTC截止时间复算为221题完成、93题正确、0 invalid、112 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_270min.log` | 153 bytes | `ba686a71c8ef662c5255874e0b29c40bc5034c5fc592191bc09e22003a73b9d3` |
| `checkpoint_270min_gpu.csv` | 520 bytes | `9eb67dbca218ed671dca5d7bf20d28a514cf42d23021a3694396fbf0a3f83ffe` |
| `checkpoint_270min_engine.log` | 714 bytes | `8e827d4cfe500b1b5b34baf72901b505de24280d4247b10aec84a9a95d1bbc57` |
| `checkpoint_270min_runtime_state.txt` | 144 bytes | `a47e57774873110b4dad0b7829a86d9875766162f8e979e42417ab7ba17d6e26` |
| `checkpoint_270min_validation.json` | 1,794 bytes | `98ca29580d574835684fdd57080ab18b1b8eb37013cf1085fedbe877a1ac6937` |
| `checkpoint_270min_manifest.sha256` | 1,704 bytes | `fd2aa62727028ce74c3a0e4365bc4f269e8dadfc85938589f82d0d3d3654639f` |
| `checkpoint_270min_manifest_check.log` | 1,022 bytes | `3c5d25ee41b138247c6f2a5eb502465ba9005c9f590ac4cae6057dc6e6c5bc7e` |

实验继续运行；280分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前221题结果提前改变候选。

### 2.299 ea8 fixed256 v5 280分钟精度节点

2.298实时记录已由主仓提交`b269cb9d9a2ff1c9d49552179f514f5523a1f097`通过GitHub HTTPS
发布，实验未中断。第二十八个固定节点由sidecar于2026-08-03T17:25:46Z写出；实际elapsed为
16,803秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 222/256 | 93 | 41.891892% | 36.328125% | 0 | 113 |

相对270分钟节点新增1题、该题未答对且发生截断；已完成题当前精度从42.081448%
降至41.891892%。剩余34题至少还需12题正确才能达到BF16的105/256正确数门禁，当前222题
结果仍不能证明终局是否达标。

17:25:57Z运行快照显示容器与tmux monitor均存活，8卡各76,149 MiB、利用率97%–100%；
最近三轮服务指标为16 running/0 waiting，生成吞吐60.8–62.4 token/s，KV cache 15.1%–15.4%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按280分钟UTC截止时间复算为222题完成、93题正确、0 invalid、113 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_280min.log` | 153 bytes | `53556f6677de9a122a0bb9aabb3657e327d88ec3bbe962e40c93384767711950` |
| `checkpoint_280min_gpu.csv` | 525 bytes | `dff0647af2a8db0a61718133ebb043d673618bd9fcc0436ebc7222f516c16ed0` |
| `checkpoint_280min_engine.log` | 714 bytes | `465006156f9f34bf68c6a6c0d87dbce9c52c151e3240d87c278b58bc149a803c` |
| `checkpoint_280min_runtime_state.txt` | 144 bytes | `b7c3b5b3af0433fec159cfd0bd2a60f3cc904efdcd78e53d2233d9c6d31474c1` |
| `checkpoint_280min_validation.json` | 1,794 bytes | `74f2c1b4237d1c3800cfafdf85e1903a9b3ce70bff64182487f6a48cf136ae44` |
| `checkpoint_280min_manifest.sha256` | 1,704 bytes | `f14e38fc740b1c1405657e6f36652a78c90dfa2bf32924ee3f9b3fb86174c966` |
| `checkpoint_280min_manifest_check.log` | 1,022 bytes | `6723a3bd3155007ba5bc5993b7e66667c941cfe116001b8986f1b8fb5cb50109` |

实验继续运行；290分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前222题结果提前改变候选。

### 2.300 ea8 fixed256 v5 290分钟精度节点

2.299实时记录已由主仓提交`fb21c3a6f48d5dd89c7bc5e2c8cd3a339398bda6`通过GitHub HTTPS
发布，实验未中断。第二十九个固定节点由sidecar于2026-08-03T17:35:46Z写出；实际elapsed为
17,403秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 237/256 | 98 | 41.350211% | 38.281250% | 0 | 123 |

相对280分钟节点新增15题，其中5题正确、10题新增截断；已完成题当前精度从41.891892%
降至41.350211%。剩余19题至少还需7题正确才能达到BF16的105/256正确数门禁，当前237题
结果仍不能证明终局是否达标。

17:35:59Z运行快照显示容器与tmux monitor均存活，8卡各76,149 MiB、利用率93%–98%；
最近三轮服务指标为16 running/0 waiting，生成吞吐68.7–76.8 token/s，KV cache 8.1%–8.2%。
日志扫描未发现Traceback、fatal、OOM或RuntimeError。

独立validator按290分钟UTC截止时间复算为237题完成、98题正确、0 invalid、123 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_290min.log` | 153 bytes | `f924066e0aaeeb2063c3df24da8e971a3d8553760dfd65009c222a4c8e4a7412` |
| `checkpoint_290min_gpu.csv` | 520 bytes | `0a2370889994eb2107ef835c6d9c526cfbfe2700e6de7edb4f54618c46a4448c` |
| `checkpoint_290min_engine.log` | 713 bytes | `94ae478e40badc218b6118f0a97903fe57f31063709df99efd172b7283fcbb09` |
| `checkpoint_290min_runtime_state.txt` | 144 bytes | `46604b13291b6a43b442c73505f03684dd1346e833dee4bbf4360d99a8973ced` |
| `checkpoint_290min_validation.json` | 1,794 bytes | `c151da29977118733f907b0be3aad6d4305087c95af63e1ed7b9cd9c7f06f3a6` |
| `checkpoint_290min_manifest.sha256` | 1,704 bytes | `91e8ddd5d77f1fba20882397c9c4fecf094b89c7f23b98e3bef7e2ced97eb9b7` |
| `checkpoint_290min_manifest_check.log` | 1,022 bytes | `b4d2d55ef93642d3403dab92f8e132b843bda5429aa97aebbe7f5d229866841e` |

实验继续运行；300分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前237题结果提前改变候选。

### 2.301 ea8 fixed256 v5 300分钟精度节点

2.300实时记录已由主仓提交`07a65640923bbdba13c8528cd560fa7a9cd58ef7`通过GitHub HTTPS
发布，实验未中断。第三十个固定节点由sidecar于2026-08-03T17:45:46Z写出；实际elapsed为
18,003秒：

| 已完成 | 正确 | 已完成题当前精度 | 全256题精度 | invalid | truncated |
|---:|---:|---:|---:|---:|---:|
| 244/256 | 100 | 40.983607% | 39.062500% | 0 | 128 |

相对290分钟节点新增7题，其中2题正确、5题新增截断；已完成题当前精度从41.350211%
降至40.983607%。剩余12题至少还需5题正确才能达到BF16的105/256正确数门禁，当前244题
结果仍不能证明终局是否达标。

17:46:00Z运行快照显示容器与tmux monitor均存活，8卡各76,153 MiB、利用率98%–100%；
尾批12 running/0 waiting，最近三轮生成吞吐44.4–56.4 token/s，KV cache 6.8%–6.9%。

独立validator按300分钟UTC截止时间复算为244题完成、100题正确、0 invalid、128 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_300min.log` | 154 bytes | `42a7d4aedcb0f124faba0225e3bd97cfe277d76d6e2d771e59265a700435a608` |
| `checkpoint_300min_gpu.csv` | 521 bytes | `62fa2f93edb890ead40831f072553cf5c45c2abfff512fbd051d260874699a18` |
| `checkpoint_300min_engine.log` | 711 bytes | `d9ede320cd5e2ea48e223d8fe7ac5d5e9c3e32ba8c1b5c6f191600b861319958` |
| `checkpoint_300min_runtime_state.txt` | 144 bytes | `57492277e8ecf4ac3b2a864865f7bd78d78232ed82fb92372648592d1d82d74a` |
| `checkpoint_300min_validation.json` | 1,796 bytes | `71fb70300732e21993bdd36723d1ee7a5f391f81128a12b1640ac4e1ee23b9ab` |
| `checkpoint_300min_manifest.sha256` | 1,704 bytes | `e228f3ce622f95a90959370c8d9dd534ba22345721c6c4085a790df3fcbc8a7e` |
| `checkpoint_300min_manifest_check.log` | 1,022 bytes | `3c33ecc6914f8445ef26b0d794eaf35b523a92e464aeaa9f9f15cd06d542014f` |

实验继续运行；310分钟节点继续使用同一固定口径。达到BF16 105/256正确数前禁止启动32K/batch1
性能复测，也不根据当前244题结果提前改变候选。

### 2.302 ea8 fixed256 v5 310分钟精度节点

2.301实时记录已由主仓提交`b9031994841d5ea8e0f81437f04d2e5e158bb078`通过GitHub HTTPS
发布，实验未中断。第三十一个固定节点由sidecar于2026-08-03T17:55:46Z写出；实际elapsed为
18,603秒：
...
相对300分钟节点无新增完成题。剩余12题至少还需5题正确才能达到BF16的105/256正确数门禁，
当前结果仍不能证明终局是否达标。

17:55:56Z单点GPU利用率恰为0%，但8卡仍各占用76,155 MiB，容器与tmux monitor均存活；
其前7–27秒的三轮服务指标持续为12 running/0 waiting，生成吞吐44.4–55.2 token/s，
KV cache 11.6%–11.8%，因此该单点0%不能作为服务停止证据。

独立validator按310分钟UTC截止时间复算为244题完成、100题正确、0 invalid、128 truncated，
14/14 checks passed；immutable manifest覆盖11项并从项目根复算11/11全部`OK`。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_310min.log` | 154 bytes | `0e1a2a928172eba7778adb4db9348bcaf567132a565769d72837d555d6ff8864` |
| `checkpoint_310min_gpu.csv` | 510 bytes | `e1668766153e4f4886a47f74af2cba6cecad28e3cbfcf42d05c972864a938f51` |
| `checkpoint_310min_engine.log` | 714 bytes | `b5fe18f9e95e168dc56bea3639af85ed22c4125f4d3084d2c611bbb39a52e3b4` |
| `checkpoint_310min_runtime_state.txt` | 144 bytes | `d842c0ca221d65d55c856d6da4880ee5109736624719c76dac0689360b79c394` |
| `checkpoint_310min_validation.json` | 1,796 bytes | `297cfb4832cd996c632b8987e682ab9eed3bba9dfb9058be27e331f808fcd960` |
| `checkpoint_310min_manifest.sha256` | 1,704 bytes | `58acb65d8e35cc962876b1df0de8d67bc044484a215618d02397f213c400280f` |
| `checkpoint_310min_manifest_check.log` | 1,022 bytes | `7c65d30bebfb2996aa2203dffcd411d971e917bc1daf9261bc63f7e29650895c` |
...
### 2.319 ea8 canonical fixed256 v2的30分钟精度

2.318的20分钟节点已由主仓提交
`beb4a6e73d769b191601b0774912388412a87e3e`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T20:03:22Z固定1,800秒截止点输出：

```text
completed=12/256 correct=8 current_accuracy=66.666667% full_set_accuracy=3.125000% invalid=0 truncated=0
```

相对20分钟节点没有新完成样本，累计结果与逐题截止快照仍严格一致。没有新增不代表实验停止：
20:04:32Z目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16 accuracy runner均存活，
8卡显存仍为76,073 MiB。该时点利用率单点恰为0%，但20:00:14Z的最近服务进度快照中8卡
利用率为63%–98%、`ready=1`且`server_completed=12/256`；因此只能记录“当前16个长请求尚未
返回”，不能把单点0%解释为退出或死锁。

运行进程参数仍包含`--hf-overrides {"index_topk":1024}`；corrected monitor已推进到
`last_emitted_elapsed=1800`、`next_elapsed=2400`，服务日志无fatal traceback/ERROR。独立
validator完成26/26 checks passed；30分钟manifest覆盖8项并复算8/8全部`OK`。当前累计正确
仍为8，不能触发105/256性能复测门禁。
...
| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_30min.log` | 147 bytes | `b51a773aaab5668edd1125fb1f4c504decab69aca975244a455cd12a65a93939` |
| `checkpoint_30min_cutoff_rows.json` | 1,949 bytes | `4bf8f77965f79f6c9a438192cba5c05202e5e6efbbe9879b21d64a5ca5db3196` |
| `checkpoint_30min_gpu.csv` | 96 bytes | `6b3ca4a92f8baacc3b66e75f8e977bec2926779a64a31b5e0694f27397814835` |
| `checkpoint_30min_validation.json` | 3,645 bytes | `16fe17b246002e8aa960e522b2f20dcac1d68d384dc0f903a60e31ef3c10e146` |
| `checkpoint_30min_manifest.sha256` | 768 bytes | `4223b83d183bb7872d0c5d1583c03ec4bb1bc035ce77913891589f87e1a6e27d` |
| `checkpoint_30min_manifest_check.log` | 272 bytes | `d121ca033e29960455fdce23ca167a0a89d1d4ec961d82ed8611260f0c1fc4bf` |
...
### 2.322 ea8 canonical fixed256 v2的60分钟精度

2.321的50分钟节点已由主仓提交
`9f106a1bd40b427c42f770ee08cce6783fe3ccf3`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T20:33:22Z固定3,600秒截止点输出：

```text
completed=34/256 correct=12 current_accuracy=35.294118% full_set_accuracy=4.687500% invalid=0 truncated=16
```

相对50分钟节点没有新完成样本，累计精度和截断数不变。逐题截止快照仍复算为34题、12题正确、
0 invalid、16 truncated，与sidecar输出一致。该节点无新增不能解释为实验停止：20:34:40Z的
运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的accuracy runner
均存活；8卡显存均为76,085 MiB，利用率为80%–98%，表明当前长请求仍在持续计算。

运行参数仍包含`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=3600`、`next_elapsed=4200`。服务保持ready，20:30:14Z最近进度快照
仍观测到`server_completed=34/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；60分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_60min.log` | 149 bytes | `ba7878c9ea7c6c9b1776bda7ff3186a02bbd2e84530e4913dcfcaa55d22897a8` |
| `checkpoint_60min_cutoff_rows.json` | 5,397 bytes | `5c6f33c5c5ac966f4277b0724195965482efb3ece55a159ee830d2d917ed21ae` |
| `checkpoint_60min_gpu.csv` | 104 bytes | `062b9c383cbf81060320252de7a07d4be3f95d5aaaabb544326118b5cb337bdb` |
| `checkpoint_60min_validation.json` | 3,653 bytes | `1227e73b827af59c8b81042e2822a609b402437655f4f4e160c5d4052aed0e21` |
| `checkpoint_60min_manifest.sha256` | 768 bytes | `15884663870dd2aad22c848c594ef01fbb6e5b743e09f1e6c82b4bb1620aefdf` |
| `checkpoint_60min_manifest_check.log` | 272 bytes | `386f28b4cc8dd99408dc2eefdd30511fa960f17d9d369ffc407e600adea1f929` |

下一步先发布本60分钟节点；实验继续到70分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.323 ea8 canonical fixed256 v2的70分钟精度

2.322的60分钟节点已由主仓提交
`cc1c1a426b54ccd55b72853efda6cd9e91597262`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T20:43:22Z固定4,200秒截止点输出：

```text
completed=48/256 correct=16 current_accuracy=33.333333% full_set_accuracy=6.250000% invalid=0 truncated=26
```

相对60分钟节点新增14题，其中4题正确；累计截断由16题增加到26题，即新增10题达到长度上限且
这些截断题均得分0。当前已完成题精度由35.294118%降至33.333333%，全量精度由4.687500%
增至6.250000%。逐题截止快照复算为48题、16题正确、0 invalid、26 truncated，与sidecar
输出完全一致；当前仍是部分样本结果，不能外推为终局。

20:44:23Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,087 MiB，利用率为54%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=4200`、`next_elapsed=4800`。服务保持ready，20:40:14Z最近服务进度
快照无fatal traceback/ERROR。

独立validator完成26/26 checks passed；70分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_70min.log` | 149 bytes | `bfa33747776fbd1099ad3ba9e7f08edef565dab4eed4461d95e95bebe6c48b99` |
| `checkpoint_70min_cutoff_rows.json` | 7,596 bytes | `dd134da41d5e17a1c1a7df27fc7148f308e47a17224204beada67dadd210c86f` |
| `checkpoint_70min_gpu.csv` | 104 bytes | `88cf2fe3a8bf9a00d55b2e75d3a0eb7090fdced1beb94ca5181bcb3df1ff8089` |
| `checkpoint_70min_validation.json` | 3,650 bytes | `2d6481e4a0fca0f3ad3991afe18daaf242239d8b63e0ccda9ba326d954dc40f8` |
| `checkpoint_70min_manifest.sha256` | 768 bytes | `192b8d0af536a2d0abc840e14265355fb982f62c29018dd695249266f4517514` |
| `checkpoint_70min_manifest_check.log` | 272 bytes | `275ae6029436c352f38eb87fe03f2fa7884ac81c6053a4469cf427712e5f91dc` |

下一步先发布本70分钟节点；实验继续到80分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.324 ea8 canonical fixed256 v2的80分钟精度

2.323的70分钟节点已由主仓提交
`c0f07d892359a3d8cdd48ba7c013927294bc20f8`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T20:53:22Z固定4,800秒截止点输出：

```text
completed=67/256 correct=25 current_accuracy=37.313433% full_set_accuracy=9.765625% invalid=0 truncated=32
```

相对70分钟节点新增19题，其中9题正确；累计截断由26题增加到32题，即新增6题达到长度上限且
这些截断题均得分0。当前已完成题精度由33.333333%升至37.313433%，全量精度由6.250000%
增至9.765625%。逐题截止快照复算为67题、25题正确、0 invalid、32 truncated，与sidecar
输出完全一致；当前仍是部分样本结果，不能外推为终局。

20:53:45Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,089 MiB，利用率为80%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=4800`、`next_elapsed=5400`。服务保持ready，20:50:15Z最近服务进度
快照已观测到`server_completed=65/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；80分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_80min.log` | 149 bytes | `f5a7bafc3c90e155236c6b2b8fb03bbc37d325095ab05de2a9029e1f3cf37607` |
| `checkpoint_80min_cutoff_rows.json` | 10,584 bytes | `093e8f5be5519555330204a78392130fe64d1c2ef75ded881134623cfee379ad` |
| `checkpoint_80min_gpu.csv` | 104 bytes | `86a0a6e01e687c132315e3f8a78f15311e58d7e1f7f8418d8865bf669b765b47` |
| `checkpoint_80min_validation.json` | 3,653 bytes | `58e5800b45e685f6332a2ef1420d42aba7fe0fa66418dbd968a4b832bc2f4d51` |
| `checkpoint_80min_manifest.sha256` | 768 bytes | `017fe286746e439592ec621a5978a32e48598c80f864c2c177dfa2b1029eff85` |
| `checkpoint_80min_manifest_check.log` | 272 bytes | `ca8e74d59f6ecc44a71eaed40e2dd574711a3adb4fc233b2264475f5c46423fe` |
...
### 2.326 ea8 canonical fixed256 v2的100分钟精度

2.325的90分钟节点已由主仓提交
`a84cf0eabccdda14e141c6769438d2b76eb77717`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T21:13:22Z固定6,000秒截止点输出：

```text
completed=77/256 correct=28 current_accuracy=36.363636% full_set_accuracy=10.937500% invalid=0 truncated=40
```

相对90分钟节点新增10题，其中3题正确；累计截断由32题增加到40题，即新增8题达到长度上限。
逐题截止快照显示40个累计截断样本中1题仍被评分为正确，因此不能再把“截断”等同于“必然错误”。
当前已完成题精度由37.313433%降至36.363636%，全量精度由9.765625%增至10.937500%。逐题
复算为77题、28题正确、0 invalid、40 truncated，与sidecar输出完全一致；当前仍是部分样本结果。

21:13:48Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,091 MiB，利用率为60%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=6000`、`next_elapsed=6600`。服务保持ready，21:10:15Z最近服务进度
快照无fatal traceback/ERROR。

独立validator完成26/26 checks passed；100分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_100min.log` | 150 bytes | `2f350cc4f7818f162036ae57ecb07a145179a7d1bf0f2d03f6ccb5b291fe83ea` |
| `checkpoint_100min_cutoff_rows.json` | 12,152 bytes | `e598bf6b85eaa48a19d8ee3d80becf1dfc989fc9d00c18b74f60b2180dbd6068` |
| `checkpoint_100min_gpu.csv` | 104 bytes | `d0149e70ee7a0daaae9778db9a7578c773aa07f9006b980ab8d4872526ed8538` |
| `checkpoint_100min_validation.json` | 3,654 bytes | `8eb01557e496f9873dfa3912d337e8b18be01cfdd256e345245e518f53aa3508` |
| `checkpoint_100min_manifest.sha256` | 776 bytes | `6cbed774990c4e7b87524f36f46a0fc7bad2636c8545519cb9d8c1ac7a3c55a6` |
| `checkpoint_100min_manifest_check.log` | 280 bytes | `1fd45963183b840d45b01e18f4384309c5cd63f8f476b22c640f3521a3d25dc6` |

下一步先发布本100分钟节点；实验继续到110分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.327 ea8 canonical fixed256 v2的110分钟精度

2.326的100分钟节点已由主仓提交
`1f1af26c6360b434eca6a4f4d4dd648fa46e13a6`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T21:23:22Z固定6,600秒截止点输出：

```text
completed=101/256 correct=40 current_accuracy=39.603960% full_set_accuracy=15.625000% invalid=0 truncated=48
```

相对100分钟节点新增24题，其中12题正确；累计截断由40题增加到48题，即新增8题达到长度上限。
逐题截止快照中48个累计截断样本有2题评分正确。当前已完成题精度由36.363636%升至
39.603960%，全量精度由10.937500%增至15.625000%。逐题复算为101题、40题正确、0 invalid、
48 truncated，与sidecar输出完全一致；已完成101题不等于正确105题，当前仍不能触发性能复测门禁。

21:24:20Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,095 MiB，利用率为72%–98%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=6600`、`next_elapsed=7200`。服务保持ready，21:20:15Z最近服务进度
快照已观测到`server_completed=97/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；110分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_110min.log` | 151 bytes | `d33bfdaea4afcf93e3cf4d110b2ac10b29d1f71e3600d5d5359cebee14e51161` |
| `checkpoint_110min_cutoff_rows.json` | 15,932 bytes | `9b6833895f2602864519c8d519fd76ffa4d43802410075ad097cc3a8f75a1d57` |
| `checkpoint_110min_gpu.csv` | 104 bytes | `befa3f76d0a641036b6af2052d1a6771c1e85b835e0155cd0701be0a507621ad` |
| `checkpoint_110min_validation.json` | 3,656 bytes | `c363734ec08dcea64ce55ad9ce48752d400cf98e6f5323fa7ef52bdc52c7b3e7` |
| `checkpoint_110min_manifest.sha256` | 776 bytes | `837c137e9efcfd652f8af1b7dd6bbf6e5113c7b142cd0e41d3d5f944d8d5ea61` |
| `checkpoint_110min_manifest_check.log` | 280 bytes | `e0952567c476f855fd5e3c572ac8ee71f716421106665c23a5d57a059486d070` |

下一步先发布本110分钟节点；实验继续到120分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.328 ea8 canonical fixed256 v2的120分钟精度

2.327的110分钟节点已由主仓提交
`885e460fdbc46943e9df1dd112e2297383adb7d6`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T21:33:22Z固定7,200秒截止点输出：

```text
completed=102/256 correct=41 current_accuracy=40.196078% full_set_accuracy=16.015625% invalid=0 truncated=48
```

相对110分钟节点新增1题且该题答对，没有新增截断。当前已完成题精度由39.603960%升至
40.196078%，全量精度由15.625000%增至16.015625%。逐题截止快照复算为102题、41题正确、
0 invalid、48 truncated，其中2个截断样本评分正确；与sidecar输出完全一致。当前累计正确数41
仍远低于105题门禁，不能启动性能复测。

21:34:14Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,095 MiB，利用率为74%–98%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=7200`、`next_elapsed=7800`。服务保持ready，21:30:16Z最近服务进度
快照已观测到`server_completed=102/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；120分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_120min.log` | 151 bytes | `8cadd21cfbb5cac7d7133e37ac5062cdd39c1f97c11cceda85c548d27422a4a1` |
| `checkpoint_120min_cutoff_rows.json` | 16,090 bytes | `a590f18092743040ce5be0e394a4f1bb5e81764719e69b0696f305169921053b` |
| `checkpoint_120min_gpu.csv` | 104 bytes | `3e159c4f96e85aa77b618347f0d8de5dfba6c4986118eb5a18b6018c9a1e4a44` |
| `checkpoint_120min_validation.json` | 3,658 bytes | `49d04b74d102d81d616cb866496dd4848afdd3287c0beb8b393b64c98dd545cf` |
| `checkpoint_120min_manifest.sha256` | 776 bytes | `a782ec4e8f2a10f86606f79b703d39f6c968e21c86d13eedeafa9434a3018ecc` |
| `checkpoint_120min_manifest_check.log` | 280 bytes | `05d8a3bafa52e4403449665a6707da209cc17bf5a5dbaf5dac727b65b541cc70` |

下一步先发布本120分钟节点；实验继续到130分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.329 ea8 canonical fixed256 v2的130分钟精度

2.328的120分钟节点已由主仓提交
`5092197659d445199ccada858f4b29ce7ad4e1ec`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T21:43:22Z固定7,800秒截止点输出：

```text
completed=111/256 correct=42 current_accuracy=37.837838% full_set_accuracy=16.406250% invalid=0 truncated=56
```

相对120分钟节点新增9题，其中1题答对，并新增8个截断样本。当前已完成题精度由40.196078%
降至37.837838%，全量精度由16.015625%增至16.406250%。逐题截止快照复算为111题、42题
正确、0 invalid、56 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数42仍远低于105题门禁，不能启动性能复测。

21:44:32Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,095 MiB，该次瞬时利用率均为0%。这一个瞬时采样不能
判定实验停止：21:40:16Z最近服务进度快照已观测到`server_completed=103/256`，同一快照8卡
利用率为77%–98%；运行参数仍包含`--hf-overrides {"index_topk":1024}`，corrected monitor已
推进至`last_emitted_elapsed=7800`、`next_elapsed=8400`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；130分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_130min.log` | 151 bytes | `2126a04d56318b27c7b22c70272318fc149e7d24a02705ce0d5f5b84d2ede261` |
| `checkpoint_130min_cutoff_rows.json` | 17,502 bytes | `5e6536dc8becf87fc5110a3f2ce6e18800350c96dc68ec508f9e8fb119013bc3` |
| `checkpoint_130min_gpu.csv` | 96 bytes | `0e5748a9db7a4bcbcd6f30f134f40d364445fc34d7fc5d6b77de071e32475a11` |
| `checkpoint_130min_validation.json` | 3,657 bytes | `f7ab5cb793590587a7d5f28912cb9c3b9b99e117e5ce6bc52824f08bcebcba9d` |
| `checkpoint_130min_manifest.sha256` | 776 bytes | `2f664b9117f6f9bd4ea76090097ac66653ff58dbe4fed27c5e4ad13ecd7de1f5` |
| `checkpoint_130min_manifest_check.log` | 280 bytes | `d233b928b7c53cdcf378fc1a66ca5210b8becd30d829f0d8a6a32fc0295a5d18` |

下一步先发布本130分钟节点；实验继续到140分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.330 ea8 canonical fixed256 v2的140分钟精度

2.329的130分钟节点已由主仓提交
`ae0d1ec8f0ca313a57cc383d7f1de868a1a4e6ed`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T21:53:22Z固定8,400秒截止点输出：

```text
completed=133/256 correct=48 current_accuracy=36.090226% full_set_accuracy=18.750000% invalid=0 truncated=64
```

相对130分钟节点新增22题，其中6题答对，并新增8个截断样本。当前已完成题精度由37.837838%
降至36.090226%，全量精度由16.406250%增至18.750000%。逐题截止快照复算为133题、48题
正确、0 invalid、64 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数48仍低于105题门禁，不能启动性能复测。

21:54:13Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,097 MiB，利用率为70%–98%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=8400`、`next_elapsed=9000`。服务保持ready，21:50:16Z最近服务进度
快照已观测到`server_completed=128/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；140分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_140min.log` | 151 bytes | `31aa6728a29175efc8531f9d3019c8ea683e33d5e4065fbd1ce1f0fadca2244d` |
| `checkpoint_140min_cutoff_rows.json` | 20,965 bytes | `d3a98c97f09a9f19aa72e7a3778f9d5f0daec25d2dd151ee18866573c78f2726` |
| `checkpoint_140min_gpu.csv` | 104 bytes | `80a0cb8ff5b51742312f9ad222900c82f2653647edc3c0e391cc9816b2b4630d` |
| `checkpoint_140min_validation.json` | 3,654 bytes | `0f7e79035d92b59e5ba1920a7b757c6b9164e25265a58f025f278784ae7bc251` |
| `checkpoint_140min_manifest.sha256` | 776 bytes | `c02098b4a938c1896cbd0ef54574d71cd9247ae17f980c5109ac232ce6726ed2` |
| `checkpoint_140min_manifest_check.log` | 280 bytes | `ab59f8a5904ebda71e2c9cca14d02ed43f88ee5b1a5f8d849a0c160a25a5ecde` |

下一步先发布本140分钟节点；实验继续到150分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.331 ea8 canonical fixed256 v2的150分钟精度

2.330的140分钟节点已由主仓提交
`def16597ff245c9ceee6c8e37671a60ade663300`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T22:03:22Z固定9,000秒截止点输出：

```text
completed=138/256 correct=52 current_accuracy=37.681159% full_set_accuracy=20.312500% invalid=0 truncated=64
```

相对140分钟节点新增5题，其中4题答对，没有新增截断。当前已完成题精度由36.090226%升至
37.681159%，全量精度由18.750000%增至20.312500%。逐题截止快照复算为138题、52题正确、
0 invalid、64 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前累计
正确数52仍低于105题门禁，不能启动性能复测。

22:03:54Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,099 MiB，利用率为76%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=9000`、`next_elapsed=9600`。服务保持ready，22:00:16Z最近服务进度
快照已观测到`server_completed=138/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；150分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_150min.log` | 151 bytes | `162d9c39c22107ff3cc9b4d53cef0ff36af27b80d9203fe435406e9f65309c48` |
| `checkpoint_150min_cutoff_rows.json` | 21,754 bytes | `7214c8bbb6357f4b7acdfacea538bd65cd7263504703b0dad0444145bb6d7d2b` |
| `checkpoint_150min_gpu.csv` | 104 bytes | `93265a32e88b7c40755f6376ec6beeec56d68f77b9e5e99d13d50b7b88d7514e` |
| `checkpoint_150min_validation.json` | 3,657 bytes | `3ab4ca9066d15254942faea3a737e5b5511d5c367752d5be85a3a1d1726e1e47` |
| `checkpoint_150min_manifest.sha256` | 776 bytes | `42ca445f775025bb44d3f582cd827ec3ea27f56cbb646c03f0e70f1c65f07fc9` |
| `checkpoint_150min_manifest_check.log` | 280 bytes | `d8718d74b6596d17391c59078521eacc51d71903dc94537f992f3c2b372842aa` |
...
### 2.333 ea8 canonical fixed256 v2的170分钟精度

2.332的160分钟节点已由主仓提交
`62f95b0eebed5470ee895b253a84de5eacb7a923`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T22:23:22Z固定10,200秒截止点输出：

```text
completed=162/256 correct=60 current_accuracy=37.037037% full_set_accuracy=23.437500% invalid=0 truncated=78
```

相对160分钟节点新增18题，其中7题答对，并新增9个截断样本。当前已完成题精度由36.805556%
升至37.037037%，全量精度由20.703125%增至23.437500%。逐题截止快照复算为162题、60题
正确、0 invalid、78 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数60仍低于105题门禁，不能启动性能复测。

22:23:54Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,101 MiB，利用率为69%–99%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=10200`、`next_elapsed=10800`。服务保持ready，22:20:17Z最近服务进度
快照已观测到`server_completed=160/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；170分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_170min.log` | 152 bytes | `83f920be671554b24fe3b001e4a00c56d2df243057cec47865564f6436cde0ba` |
| `checkpoint_170min_cutoff_rows.json` | 25,526 bytes | `913816832b6474b64ece2c04ee3dd06466980a58b6a9612b1dd8267d2c9a916b` |
| `checkpoint_170min_gpu.csv` | 104 bytes | `bd7fd6a4c9e65da049e238f337a454975443ad2b2f5eaa59420865fc96b3d362` |
| `checkpoint_170min_validation.json` | 3,657 bytes | `0142fb98fa862c408b5397d5a5048d1900dff2dfa9a64d7b2f45cdfadee4bbc2` |
| `checkpoint_170min_manifest.sha256` | 776 bytes | `ebe8e81c76b2ec86a8b008c29da90e5cf2f9e7629b6780e221c30fbf60e64cd1` |
| `checkpoint_170min_manifest_check.log` | 280 bytes | `d4084799e814bfcf875e6acbcf3ede01f3c37e3391ea85a563d96c170a18906c` |

下一步先发布本170分钟节点；实验继续到180分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.334 ea8 canonical fixed256 v2的180分钟精度

2.333的170分钟节点已由主仓提交
`4f91b00caf5ae9d5b09514aa79b0e1fc565b4023`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T22:33:22Z固定10,800秒截止点输出：

```text
completed=166/256 correct=62 current_accuracy=37.349398% full_set_accuracy=24.218750% invalid=0 truncated=80
```

相对170分钟节点新增4题，其中2题答对，并新增2个截断样本。当前已完成题精度由37.037037%
升至37.349398%，全量精度由23.437500%增至24.218750%。逐题截止快照复算为166题、62题
正确、0 invalid、80 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数62仍低于105题门禁，不能启动性能复测。

22:33:53Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,103 MiB，利用率为72%–98%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=10800`、`next_elapsed=11400`。服务保持ready，22:30:17Z最近服务进度
快照已观测到`server_completed=166/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；180分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_180min.log` | 152 bytes | `00d733e0a000ca39399fe9f3409ea0a3152539bd3ee585d053f731719cb7be83` |
| `checkpoint_180min_cutoff_rows.json` | 26,156 bytes | `7e191543bc14f9c767e049d2dfb93443bd27ca3f96a82762048ac3a4b54e67eb` |
| `checkpoint_180min_gpu.csv` | 104 bytes | `5d7e91d20dd8fbfc270daf620743f1c8c351b2c79b70777842129a16eea5b711` |
| `checkpoint_180min_validation.json` | 3,658 bytes | `c86b42c78fb7504786a8a4089b9bf4a2f6b57e6b8abadf4d324190cd12d77b64` |
| `checkpoint_180min_manifest.sha256` | 776 bytes | `439a185412e98b3a5c40bc519923f9c98d7d76a5924cb53651df18d4ae536a1f` |
| `checkpoint_180min_manifest_check.log` | 280 bytes | `b8f36122464dbdbd6b99846c15d2c84b89d7b7c3dda01e47dadb7662c17acba3` |

下一步先发布本180分钟节点；实验继续到190分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.335 ea8 canonical fixed256 v2的190分钟精度

2.334的180分钟节点已由主仓提交
`ef13ccf6fc279837dffb0151025d6618d37119a0`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T22:43:22Z固定11,400秒截止点输出：

```text
completed=175/256 correct=67 current_accuracy=38.285714% full_set_accuracy=26.171875% invalid=0 truncated=84
```

相对180分钟节点新增9题，其中5题答对，并新增4个截断样本。当前已完成题精度由37.349398%
升至38.285714%，全量精度由24.218750%增至26.171875%。逐题截止快照复算为175题、67题
正确、0 invalid、84 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数67仍低于105题门禁，不能启动性能复测。

22:43:50Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,103 MiB，利用率为71%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=11400`、`next_elapsed=12000`。服务保持ready，22:40:17Z最近服务进度
快照已观测到`server_completed=169/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；190分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_190min.log` | 152 bytes | `ea2b23f2ed49fd34e00cea930dd7a74fc62a14223fa2e5d4f0289e7c7af469ad` |
| `checkpoint_190min_cutoff_rows.json` | 27,573 bytes | `854a547bc30152b2a8b24856d8717d6a0f9b2a51cefaf2c238fa3bf935006d0d` |
| `checkpoint_190min_gpu.csv` | 104 bytes | `d22fb4c985e7c4db08f13edb079cfac0c85a89dac23165efa77eadf35bcdf9af` |
| `checkpoint_190min_validation.json` | 3,659 bytes | `37012620c8b7cef73438a6b0f4bb2ee3e1cd69bd87f090edd39bfcf3eb51903b` |
| `checkpoint_190min_manifest.sha256` | 776 bytes | `7e14fc04a6f8ca05ac5c7bc5e64d45b9672339c175c54dcb2d2de291ffed310a` |
| `checkpoint_190min_manifest_check.log` | 280 bytes | `03ccccd12679e127cd146d9fb8a84fa8fe3f9ef2429ed4692769eb848c5eaeb3` |

下一步先发布本190分钟节点；实验继续到200分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.336 ea8 canonical fixed256 v2的200分钟精度

2.335的190分钟节点已由主仓提交
`fce323d4752f732b007562417014de5993fd046b`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T22:53:22Z固定12,000秒截止点输出：

```text
completed=196/256 correct=75 current_accuracy=38.265306% full_set_accuracy=29.296875% invalid=0 truncated=94
```

相对190分钟节点新增21题，其中8题答对，并新增10个截断样本。当前已完成题精度由38.285714%
降至38.265306%，全量精度由26.171875%增至29.296875%。逐题截止快照复算为196题、75题
正确、0 invalid、94 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数75仍低于105题门禁，不能启动性能复测。

22:53:52Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,105 MiB，利用率为72%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=12000`、`next_elapsed=12600`。服务保持ready，22:50:17Z最近服务进度
快照已观测到`server_completed=193/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；200分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_200min.log` | 152 bytes | `969b060beed5f65e16d7afa33c2d3c7ce86638d6957a93a4c6fe77f968f2d063` |
| `checkpoint_200min_cutoff_rows.json` | 30,876 bytes | `f7d9c01dc1daf82cb07ef17bfc2a69d2ccb1653de449b98b8a50965259816a8c` |
| `checkpoint_200min_gpu.csv` | 104 bytes | `795f8416fbd92067e24c5f06e00405123ae76c1ef2818763aba555545050aa1c` |
| `checkpoint_200min_validation.json` | 3,658 bytes | `e53c809b7876ada90e2a7e1ec089543cdb7c4480b8a30df315105e97562caa3d` |
| `checkpoint_200min_manifest.sha256` | 776 bytes | `1942b892d275452522db8d0d909f2f8167440a4dfec2563cf56ca8522f2d9ae2` |
| `checkpoint_200min_manifest_check.log` | 280 bytes | `83e3ab3ec88fd49a4d9dd662a1ee145ebc350345959bbffdd9ca97b9df5ccaa6` |

下一步先发布本200分钟节点；实验继续到210分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.337 ea8 canonical fixed256 v2的210分钟精度

2.336的200分钟节点已由主仓提交
`141188e20a7b5c4ca13d7a42d7f02ba2b39d88fb`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T23:03:22Z固定12,600秒截止点输出：

```text
completed=201/256 correct=77 current_accuracy=38.308458% full_set_accuracy=30.078125% invalid=0 truncated=96
```

相对200分钟节点新增5题，其中2题答对，并新增2个截断样本。当前已完成题精度由38.265306%
升至38.308458%，全量精度由29.296875%增至30.078125%。逐题截止快照复算为201题、77题
正确、0 invalid、96 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数77仍低于105题门禁，不能启动性能复测。

23:04:01Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,105 MiB，利用率为74%–97%。运行参数仍包含
`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=12600`、`next_elapsed=13200`。服务保持ready，23:00:17Z最近服务进度
快照已观测到`server_completed=201/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；210分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_210min.log` | 152 bytes | `8128855009900536b10bd62344bbd1efe96005f8cef9cc597786cba251b5c427` |
| `checkpoint_210min_cutoff_rows.json` | 31,663 bytes | `05af05cfdca78666816e1f5858c0bd46975a20de1e0a8e13401b673c6b037542` |
| `checkpoint_210min_gpu.csv` | 104 bytes | `43d8e512880bf400fa0776f00646ec0255f061d27de313987b19afc98b6b58af` |
| `checkpoint_210min_validation.json` | 3,659 bytes | `aade29e331f1efe0be0627f9330dff2b456afa0d87066fcadc1e12450ae7963e` |
| `checkpoint_210min_manifest.sha256` | 776 bytes | `d4a4c28cc8d64cb56b67c41bc78d0a95751deb219e03029d5adeb7f685abebfd` |
| `checkpoint_210min_manifest_check.log` | 280 bytes | `2ff8cca4ea875a6376000ae3419e900b16116f38e344c291b2138cfc3ca10011` |

下一步先发布本210分钟节点；实验继续到220分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.338 ea8 canonical fixed256 v2的220分钟精度

2.337的210分钟节点已由主仓提交
`b4d360a88e7abaf4fd865ffc9eb7cf42c657d1e0`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T23:13:22Z固定13,200秒截止点输出：

```text
completed=207/256 correct=77 current_accuracy=37.198068% full_set_accuracy=30.078125% invalid=0 truncated=101
```

相对210分钟节点新增6题，没有新增正确，并新增5个截断样本。当前已完成题精度由38.308458%
降至37.198068%，全量精度仍为30.078125%。逐题截止快照复算为207题、77题正确、0 invalid、
101 truncated，其中仍有2个截断样本评分正确；与sidecar输出完全一致。当前累计正确数77仍低于
105题门禁，不能启动性能复测。

23:14:03Z的运行态快照中，目标容器、主tmux、corrected-monitor tmux、TP0–TP7和并发16的
accuracy runner均存活；8卡显存均为76,107 MiB，该次瞬时利用率均为0%。这一个瞬时采样不能
判定实验停止：23:10:18Z最近服务进度快照已观测到`server_completed=202/256`，同一快照8卡
利用率为76%–98%；运行参数仍包含`--hf-overrides {"index_topk":1024}`，corrected monitor已
推进至`last_emitted_elapsed=13200`、`next_elapsed=13800`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；220分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_220min.log` | 153 bytes | `5e1d510eb1527ae051cd587aab50efd4c58ef7c28b944cef1d51017f5a2601dc` |
| `checkpoint_220min_cutoff_rows.json` | 32,605 bytes | `cb1eb4266cdd62c4e6ffec81b44e11444e01c016100ca3562473541c9f7c0f7c` |
| `checkpoint_220min_gpu.csv` | 96 bytes | `317d44b0bb294c69367b1e7953dce9cbeaaf8a95d3b8fa2384b332275635c02d` |
| `checkpoint_220min_validation.json` | 3,661 bytes | `1537923a6edcfd8e9ad33e8c8a2056d8b3ed57b73f816098f86642a743f31dc9` |
| `checkpoint_220min_manifest.sha256` | 776 bytes | `f9def5b95e38515faed1b83beca4f6ef483abb65f0d6e3795d2c0e89962a7383` |
| `checkpoint_220min_manifest_check.log` | 280 bytes | `e33f58b46b89734b09a6d1f0c5682ccc2558695e027181b942b19118d32ebdef` |

下一步先发布本220分钟节点；实验继续到230分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.339 ea8 canonical fixed256 v2的230分钟精度

2.338的220分钟节点已由主仓提交
`f9af2615e47f18bee5cae031613120266c65956a`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T23:23:22Z固定13,800秒截止点输出：

```text
2026-08-03T23:23:22Z elapsed_seconds=13800 completed=227/256 correct=87 current_accuracy=38.325991% full_set_accuracy=33.984375% invalid=0 truncated=110
```

相对220分钟节点新增20题，其中10题答对，并新增9个截断样本。当前已完成题精度由37.198068%
升至38.325991%，全量精度由30.078125%升至33.984375%。逐题截止快照复算为227题、87题
正确、0 invalid、110 truncated，其中4个截断样本评分正确；与sidecar输出完全一致。当前累计
正确数87仍低于105题门禁，不能启动性能复测。

23:24:01Z的运行态快照中，目标容器已运行约4小时，主tmux、corrected-monitor tmux、
TP0–TP7和并发16的accuracy runner均存活；8卡显存均为76,109 MiB，利用率为80%–97%。
运行参数仍包含`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=13800`、`next_elapsed=14400`。服务保持ready，23:20:18Z最近服务进度
快照已观测到`server_completed=223/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；230分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_230min.log` | 153 bytes | `ebbd673c2742b5d7d6a200f66218c9405b0f7d0b33b3e8a2cc37cdf8877ed61f` |
| `checkpoint_230min_cutoff_rows.json` | 35,748 bytes | `75f65436f3c625676a37e32803b7a8c2358f6ef1811408922612246ccdf3f2e3` |
| `checkpoint_230min_gpu.csv` | 104 bytes | `381523c33726d9dad5e81f325528cf901f7325c7cf876c436fb8e10a3e08c7c3` |
| `checkpoint_230min_validation.json` | 3,661 bytes | `565194d34568482d956f267553952b7f7cbda6a0b3216a85051001cab45a2dad` |
| `checkpoint_230min_manifest.sha256` | 776 bytes | `8ba2a98d214907002a9cfced1d58ca88dcf6d56fedc1007fd9f849e7a1d8e25e` |
| `checkpoint_230min_manifest_check.log` | 280 bytes | `f7c7e0f2c82d3ca20c18c06f25499a9976ac07cc84d5dfe399240a836fb34f3c` |

下一步先发布本230分钟节点；实验继续到240分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.340 ea8 canonical fixed256 v2的240分钟精度

2.339的230分钟节点已由主仓提交
`77789ce7ac4b894be7c260f6852838fac5d098b9`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T23:33:22Z固定14,400秒截止点输出：

```text
2026-08-03T23:33:22Z elapsed_seconds=14400 completed=229/256 correct=87 current_accuracy=37.991266% full_set_accuracy=33.984375% invalid=0 truncated=112
```

相对230分钟节点新增2题，没有新增正确，并新增2个截断样本。当前已完成题精度由38.325991%
降至37.991266%，全量精度仍为33.984375%。逐题截止快照复算为229题、87题正确、0 invalid、
112 truncated，其中仍有4个截断样本评分正确；与sidecar输出完全一致。当前累计正确数87仍低于
105题门禁，不能启动性能复测。

23:34:59Z的运行态快照中，目标容器已运行约4小时，主tmux、corrected-monitor tmux、
TP0–TP7和并发16的accuracy runner均存活；8卡显存均为76,109 MiB，利用率为74%–98%。
运行参数仍包含`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=14400`、`next_elapsed=15000`。服务保持ready，23:30:18Z最近服务进度
快照已观测到`server_completed=229/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；240分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_240min.log` | 153 bytes | `d50afa455145b83f31f4a1c13dc66912037a5c943eecd03fccf7eb4ab4011266` |
| `checkpoint_240min_cutoff_rows.json` | 36,062 bytes | `806841b4ed7f97ac2b77610f11e33a42753bdcde9cb6de36aa83bca04ee6d04f` |
| `checkpoint_240min_gpu.csv` | 104 bytes | `21a13f0f982e07bfa03bb0e00f3d992541ebf9447108da63228601a828c03ea7` |
| `checkpoint_240min_validation.json` | 3,661 bytes | `4472164633fcd691e426bfb1c07c9ee48af3ecc0f6b0c4b3a3b80bb254320809` |
| `checkpoint_240min_manifest.sha256` | 776 bytes | `510b6b4b969864f6dad6e52887901ade4db21633e033aed81d50afcee584e64f` |
| `checkpoint_240min_manifest_check.log` | 280 bytes | `0d0ffea2130a3de857c60de3580ce8cad7974009a58e7adf5248fcf7389f9d8e` |

下一步先发布本240分钟节点；实验继续到250分钟固定截止点并按相同口径实时更新本记录。正式终局
达到105/256前继续禁止32K/batch1性能复测。

### 2.341 ea8 canonical fixed256 v2的250分钟精度

2.340的240分钟节点已由主仓提交
`47be031a60de1cd9900b564e17fa9fff2d47fbe0`通过GitHub HTTPS发布。corrected sidecar在
2026-08-03T23:43:22Z固定15,000秒截止点输出：

```text
2026-08-03T23:43:22Z elapsed_seconds=15000 completed=236/256 correct=90 current_accuracy=38.135593% full_set_accuracy=35.156250% invalid=0 truncated=115
```

相对240分钟节点新增7题，其中3题答对，并新增3个截断样本。当前已完成题精度由37.991266%
升至38.135593%，全量精度由33.984375%升至35.156250%。逐题截止快照复算为236题、90题
正确、0 invalid、115 truncated，其中仍有4个截断样本评分正确；与sidecar输出完全一致。当前
累计正确数90仍低于105题门禁，不能启动性能复测。

23:44:00Z的运行态快照中，目标容器已运行约4小时，主tmux、corrected-monitor tmux、
TP0–TP7和并发16的accuracy runner均存活；8卡显存均为76,111 MiB，利用率为74%–96%。
运行参数仍包含`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=15000`、`next_elapsed=15600`。服务保持ready，23:40:18Z最近服务进度
快照已观测到`server_completed=233/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；250分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_250min.log` | 153 bytes | `f469eccf4d34868e0b35247bf8cff72a578582631cb019e67e446827b1066e37` |
| `checkpoint_250min_cutoff_rows.json` | 37,159 bytes | `0dfd0e4251fe58289599e12894aa703755655b0c6e0a1bc14546fdb8a7a19c91` |
| `checkpoint_250min_gpu.csv` | 104 bytes | `b216394d125906de6c60de65f99303bbfeb70e5b8dea4ac95cd490da5ef7a1ab` |
| `checkpoint_250min_validation.json` | 3,660 bytes | `60e5affe6dababce63742b69c0f2515ef4b5cbdac628edc9cba8a3e404fc3a7d` |
| `checkpoint_250min_manifest.sha256` | 776 bytes | `fa308034215c7607f94462b6cc1a5ae80d7c7176959c6fb482226e163bf85ea2` |
| `checkpoint_250min_manifest_check.log` | 280 bytes | `ff0b0f3f3a3c5d68ee011a2c524ced834fb0488456a4609233a3c65ae55ac81d` |
...
### 2.343 ea8 canonical fixed256 v2的270分钟精度

2.342的260分钟节点及门禁失败已由主仓提交
`c0d5c1bc518857568bdd55d1e37f9f1e9c4d298d`通过GitHub HTTPS发布。corrected sidecar在
2026-08-04T00:03:22Z固定16,200秒截止点输出：

```text
2026-08-04T00:03:22Z elapsed_seconds=16200 completed=252/256 correct=93 current_accuracy=36.904762% full_set_accuracy=36.328125% invalid=0 truncated=128
```

相对260分钟节点新增2题，没有新增正确，并新增2个截断样本。当前已完成题精度由37.200000%
降至36.904762%，全量精度仍为36.328125%。逐题截止快照复算为252题、93题正确、0 invalid、
128 truncated，其中仍有4个截断样本评分正确；与sidecar输出完全一致。

当前只剩4题未完成，理论最好终局已进一步收紧为`93 + 4 = 97/256`，继续严格低于BF16
baseline的`105/256`。本节点没有改变2.342的性能门禁失败结论；32K/batch1性能复测继续禁止。

00:04:06Z的运行态快照中，目标容器已运行约5小时，主tmux、corrected-monitor tmux、
TP0–TP7和并发16的accuracy runner均存活；8卡显存均为76,117 MiB，利用率为48%–97%。
运行参数仍包含`--hf-overrides {"index_topk":1024}`；corrected monitor已推进至
`last_emitted_elapsed=16200`、`next_elapsed=16800`。服务保持ready，00:00:19Z最近服务进度
快照已观测到`server_completed=252/256`，日志无fatal traceback/ERROR。

独立validator完成26/26 checks passed；270分钟manifest覆盖8项并复算8/8全部`OK`。证据位于
`artifacts/phase9-control/20260803T2010Z_ea8_canonical_fast256_launch_v2`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_270min.log` | 153 bytes | `c938979b531158e581e78d4bcd91254d3c8dfb2c8f04deee51b6ff08f361fe5a` |
| `checkpoint_270min_cutoff_rows.json` | 39,671 bytes | `ee288c7f22137cfc4f186835df4d70b08de455878c3c126579a7d3e9c0795e7c` |
| `checkpoint_270min_gpu.csv` | 104 bytes | `aff9d37fb0a7a7d4382a34510333de26542512f62d245f037cefd210e57880a0` |
| `checkpoint_270min_validation.json` | 3,662 bytes | `bcd1a5026bb35c5e15ef894357caf559862898ce0224eb6c953163dd7e08ca4f` |
| `checkpoint_270min_manifest.sha256` | 776 bytes | `412a0ee12521c37614a36af020a89e4086cf60e445693fc50d3a3709a22db562` |
| `checkpoint_270min_manifest_check.log` | 280 bytes | `53831b8e97670593039a3f66c350b85cefe63ee37009d20b343434d415e2cf37` |
...
### 2.353 ea8 prefill top-k 1,024 fixed256 v2的30分钟精度

2.352的20分钟节点已由主仓提交
`b6fba23b32c30934c0984897f163ffbf7d09c7f8`通过GitHub HTTPS发布，实验未中断。30分钟
monitor按正式wrapper启动时间在2026-08-04T01:28:30Z固定1,800秒截止点输出：

```text
2026-08-04T01:28:30Z elapsed_seconds=1800 completed=17/256 correct=15 current_accuracy=88.235294% full_set_accuracy=5.859375% invalid=0 truncated=0
```

容器内按checkpoint文件mtime不晚于该截止点复算仍为17条唯一题目、15题正确、0 invalid、
0 truncated，protocol fingerprint仍为
`5bc5f1a00a7c48e86baf8a4e1e2b52b17ebbf2f0321b319a6e27b8ca0a404718`，与monitor完全
一致。相对20分钟节点没有新题完成；该事实本身不足以判定实验卡死，也不改变当前仅覆盖
17/256题、不能外推终局的边界。

01:30:21Z运行态快照显示容器、主tmux、TP0–TP7及并发16的fixed256 runner均存活；8卡各
76,081 MiB，利用率75%–97%，compute只包含8个目标TP worker。server已ready，日志未出现
Traceback、RuntimeError、OOM或CUDA out of memory。因此该节点有充分证据表明长请求仍在
计算，没有运行异常或卡死证据；monitor已推进到下一固定截止点2,400秒。

独立validator完成22/22 checks passed；30分钟manifest覆盖9项并从证据目录复算9/9全部
`OK`。证据位于
`artifacts/phase9-control/20260804T0100Z_ea8_prefill1024_fast256_launch_v1`：

| 文件 | 大小 | SHA256 |
|---|---:|---|
| `checkpoint_30min.log` | 148 bytes | `7411f82796614a393b498b3189da69624d2376c189c172174a60c7d345b582bb` |
| `checkpoint_30min_cutoff_rows.json` | 45,548 bytes | `6db10f46c75e30b1b8297e2a46ccbb277ef16f900d388266ce0245c50822e5cc` |
| `checkpoint_30min_gpu.csv` | 104 bytes | `0937dd47e48b0007eb7a3886821f23337b0d22e7a5ebb70853223d3412d0b622` |
| `checkpoint_30min_runtime_state.txt` | 3,964 bytes | `b7272a9d8db9494be0fea1bde96f00d95cdcd3a9139ff25d0d282fdbf8391769` |
| `checkpoint_30min_validation.json` | 3,851 bytes | `baa8678ee698f2f578796b46fb7fdea421cb469177fa620f95b38018511ad954` |
| `checkpoint_30min_manifest.sha256` | 860 bytes | `916f4cf74c31b1aa3a212c490874bdcde1ca71836eced526c1affb1003405229` |
| `checkpoint_30min_manifest_check.txt` | 316 bytes | `bc4a26814ce8ebe5134f44a2b80b9f47fcd7fb0ebb2bdbdf419581fc61e24987` |

实验继续运行；下一步先发布本30分钟节点，并在40分钟固定截止点按同一口径实时更新。终局达到
至少105/256前继续禁止32K/batch1性能复测。
